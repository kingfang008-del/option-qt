...
OUTPUT_FEATURES_DIR = Path.home() / "train_data/ret_cost_quote_features_raw"
...
class FeatureEngineer:
    ...
    def _add_vix_level(self, df: pd.DataFrame):
        ...
        try:
            ...
            # 7. 插值和填充（确保无NaN，减少零值）
            df['vix_level'] = df['vix_level'].interpolate(method='linear', limit_direction='both')
    ...
    def calculate_poc_vectorized(self, df: pd.DataFrame, window: int = 50, bins: int = 50) -> pd.Series:
        ...
        result_series.bfill(inplace=True)
    ...
    def compute_features_raw(self, df: pd.DataFrame, resolution: str):
        ...
        for feature in self.config['features']: 
            ...
            try:
                ...
                elif name == 'price_z_score':
                    df['price_z_score'] = df['close'] 
                ...
                # 新添加的领先指标
                    ...
                    # =========================================================
                    # [做市商 Raw Gamma 敞口代理 (Net GEX Proxy)]
                    # 强制 Series 类型保护，彻底解决 float 无 fillna 报错
                    # =========================================================
                    
                    # 1. 安全提取：保证拿到的一定是与 df.index 长度一致的 Series
                    vw_gamma = df['options_vw_gamma'] if 'options_vw_gamma' in df.columns else pd.Series(0.0, index=df.index)
                    pcr_vol = df['options_pcr_volume'] if 'options_pcr_volume' in df.columns else pd.Series(1.0, index=df.index)
                    
                    # 2. 计算多空筹码失衡度
                    vol_imbalance = (1.0 - pcr_vol) / (1.0 + pcr_vol + 1e-9)
                    
                    # 3. 合成净 Gamma 敞口 (此时 raw_net_gex 必定是 Series)
                    raw_net_gex = vw_gamma * vol_imbalance
                    
                    df['net_gex'] = raw_net_gex.fillna(0.0)

                elif name in ['net_vanna', 'net_charm', 'vanna_flow']:
                    # =========================================================
                    # [做市商 Vanna & Charm 净敞口与流向]
                    # 同理，增加防弹装甲
                    # =========================================================
                    ...
                    if name == 'net_vanna':
                        vw_vanna = df['options_vw_vanna'] if 'options_vw_vanna' in df.columns else pd.Series(0.0, index=df.index)
                        df['net_vanna'] = (vw_vanna * vol_imbalance).fillna(0.0)
                        
                    elif name == 'net_charm':
                        vw_charm = df['options_vw_charm'] if 'options_vw_charm' in df.columns else pd.Series(0.0, index=df.index)
                        df['net_charm'] = (vw_charm * vol_imbalance).fillna(0.0)
                        
                    elif name == 'vanna_flow':
                        # 计算 flow 依赖 net_vanna，安全起见先算一遍 net_vanna
                        vw_vanna = df['options_vw_vanna'] if 'options_vw_vanna' in df.columns else pd.Series(0.0, index=df.index)
                        net_vanna = (vw_vanna * vol_imbalance).fillna(0.0)
                        
                        iv_mom = df['options_iv_momentum'] if 'options_iv_momentum' in df.columns else pd.Series(0.0, index=df.index)
                        df['vanna_flow'] = (net_vanna * iv_mom).fillna(0.0)
    ...
    def process_atr_labels_file(df: pd.DataFrame , stock_file: str):
        """
        【简化版】加载单个已生成的特征文件，计算 label_direction, label_volatility, label_event，并直接更新这些列。
        无需外部数据合并。
        """
        try:
            ...
            if df.empty or 'timestamp' not in df.columns or 'close' not in df.columns:
                return f"[跳过] process_atr_labels_file  为空或缺少必要列（timestamp/close）。"
    
            # 确保时间戳是带时区的（如果需要）
            # 从 config 获取参数
            labeling_params = config.get('parameters', {}).get('labeling', {})
            k = labeling_params.get('k', 10)
            # 【新】用一个更大的窗口来计算回报率的滚动标准差，以获得更稳定的基准
            vol_window = labeling_params.get('vol_window', 252) 
            C_init = labeling_params.get('C', 1.25) # 新基准下的系数，通常从1.0附近开始
            
            D_init = labeling_params.get('D', 2.5)
            target_range = (70, 90)
            min_extreme_ratio = 10
            max_iters = 5
            step_reduce = 0.85
            step_increase = 1.15
            fallback_thresh = 0.001 # 固定阈值保留作为备用
        
            # --- 核心逻辑重构 ---
        
            # 1. 计算未来回报率（这是我们分析的核心）
            future_returns = (df['close'].shift(-k) - df['close']) / df['close']
            # 将其作为一个临时列加入，方便后续计算
            df['future_returns'] = future_returns
        
            # 2. 计算回报率自身的滚动标准差作为动态基准
            # 使用 .rolling().std() 计算历史回报率的波动性
            returns_volatility = df['future_returns'].rolling(window=vol_window, min_periods=int(vol_window/2)).std()
            
            # 用前向填充处理开头的NaN，使得整个序列都有一个基准波动率
            returns_volatility.fillna(method='bfill', inplace=True) 
            returns_volatility.fillna(method='ffill', inplace=True)
            # 如果整个序列波动都极小，则使用一个小的非零值，避免阈值为0
            returns_volatility.replace(0, 1e-6, inplace=True)
        
        
            # 如果基准波动率太低，则回退到固定阈值模式
            if returns_volatility.mean() < 1e-6:
                dir_thresh_series = pd.Series(fallback_thresh, index=df.index)
            else:
                # 3. 迭代寻找最优系数 C
                C = C_init
                balanced = False
                final_consol_ratio, final_extreme_ratio = 0, 0
                
                for i in range(max_iters):
                    # 新的动态阈值，与回报率自身的波动性成正比
                    dir_thresh_series = C * returns_volatility
        
                    # 【重要】正确处理NaN：只在有未来回报率的行上进行判断
                    # dropna() 会返回一个只包含有效（非NaN）回报率的Series
                    valid_returns = df['future_returns'].dropna()
                    
                    # 在有效索引上进行比较
                    up_labels = valid_returns > dir_thresh_series[valid_returns.index]
                    down_labels = valid_returns < -dir_thresh_series[valid_returns.index]
                    
                    # 计算分布
                    total_valid = len(valid_returns)
                    consol_ratio = 100 * (total_valid - up_labels.sum() - down_labels.sum()) / total_valid
                    extreme_ratio = 100 * (up_labels.sum() + down_labels.sum()) / total_valid
                    final_consol_ratio, final_extreme_ratio = consol_ratio, extreme_ratio
        
                    logging.info(f"文件 {stock_file} 迭代 {i+1}: C={C:.4f}, 盘整={consol_ratio:.2f}%, 极端={extreme_ratio:.2f}%")
        
                    if target_range[0] <= consol_ratio <= target_range[1] and extreme_ratio >= min_extreme_ratio:
                        balanced = True
                        break
                    
                    # 调整 C
                    if consol_ratio > target_range[1] or extreme_ratio < min_extreme_ratio:
                        C *= step_reduce
                    else:
                        C *= step_increase
        
                if not balanced:
                    logging.warning(f" {stock_file} 标签分布未达平衡，使用最后迭代结果。")

            # 4. 【重要】安全地赋值标签
            df['label_direction'] = 1 # 默认所有为盘整
            # 只在有有效未来回报率的索引上，根据最终的阈值进行赋值
            valid_indices = df['future_returns'].notna()
            df.loc[valid_indices & (df['future_returns'] > dir_thresh_series), 'label_direction'] = 2
            df.loc[valid_indices & (df['future_returns'] < -dir_thresh_series), 'label_direction'] = 0
        
            # 清理临时列
            df.drop(columns=['future_returns'], inplace=True)
    
    
        
            # label_volatility（不变）
            log_returns = np.log(df['close'].shift(-1) / df['close'])
            rolling_log_returns = sliding_window_view(log_returns.to_numpy(), window_shape=k)
            df['label_volatility'] = np.full(len(df), np.nan)
            #df['label_volatility'][:-k+1] = np.std(rolling_log_returns, axis=1)
            df.loc[df.index[:-k+1], 'label_volatility'] = np.std(rolling_log_returns, axis=1)
            df['label_volatility'] = df['label_volatility'].fillna(0.0)
        
            # label_event（使用动态阈值）
            rolling_high = sliding_window_view(df['high'].to_numpy(), window_shape=k)
            rolling_low = sliding_window_view(df['low'].to_numpy(), window_shape=k)
            max_high = np.max(rolling_high, axis=1)
            min_low = np.min(rolling_low, axis=1)
            price_range = (max_high - min_low) / df['close'].to_numpy()[:-k+1]
            df['label_event'] = np.full(len(df), 0)
            #df['label_event'][:-k+1] = (price_range > event_thresh[:-k+1]).astype(int)
            event_thresh_series = D_init * returns_volatility
            df.loc[df.index[:-k+1], 'label_event'] = (price_range > event_thresh_series).astype(int)
            df['label_event'] = df['label_event'].fillna(0).astype(int)
                
            return  df
        except Exception as e:
            import traceback
            return f"[错误] {stock_file}   失败: {e}\n{traceback.format_exc()}"
...
def update_atr_features_in_files(config: dict):
    """
    遍历所有已生成的特征文件，并批量添加ATR归一化特征。
    """
    ...
    logging.info(f"找到 {len(tasks)} 个特征文件，开始批量添加ATR归一化特征...")

    # 使用多进程并行更新
    worker_func = partial(process_atr_features_file, config=config)
    with concurrent.futures.ProcessPoolExecutor(max_workers=MAX_WORKERS) as executor:
        results = list(tqdm(executor.map(worker_func, tasks), total=len(tasks), desc="Updating ATR Features"))

    # 打印统计和错误日志
    failures = [res for res in results if res.startswith("[错误]")]
    if failures:
        log_file = "atr_feature_update_failures.log"
...
def generate_vix_level_global(config: dict):
    ...
    for res in resolutions:
        ...
        #df_all.rename(columns={'high': 'vix_proxy_close'}, inplace=True)
        df_all['vix_proxy_close'] = df_all['vix_proxy_close'].interpolate(method='linear', limit_direction='both').ffill()

        # --- 1. 计算去趋势化的绝对水平指标 (不变) ---
        # 窗口大小根据分辨率调整 (假设 1min 对应原有设置)
        res_factor = 1
        if 'min' in res:
        ...
        df_all['vix_level'] = df_all['vix_level'].replace(0, np.nan)
        df_all['vix_level'] = df_all['vix_level'].interpolate(method='linear', limit_direction='both').fillna(method='ffill').fillna(method='bfill')
...
def process_labels_file_back(file_path: Path, config: dict = None) -> dict:
    """
    [The Ultimate Macro-Option Engine - 30min 核心实战版]
    融合机制:
    1. Gap-Forward TWAP (防延迟与滑点)
    2. Regime-Normalized Gamma Label (IV + U-Shape 动态波动率惩罚)
    3. Convexity (1.5次方 Gamma 爆炸模拟)
    4. Cost-Friction (死水盘过滤)
    """
    try:
        # 1. [I/O] 读取数据
        ...
        # [安全补丁 1] 强制确保 timestamp 是 datetime 类型，防止 .dt 报错
        if 'timestamp' in df.columns:
            if not pd.api.types.is_datetime64_any_dtype(df['timestamp']):
                df['timestamp'] = pd.to_datetime(df['timestamp'])
            df = df.sort_values('timestamp').reset_index(drop=True)
        else:
            return {'status': 'skipped', 'path': str(file_path), 'message': 'No timestamp column'}
        ...
        # 波动率基准窗口: 用过去 2 小时 (120 min) 衡量宏观风险环境
        vol_window = 120 
        ...
        # ==============================================================================
        # 3. [Barrier Label Generation] (Direction & Event)
        # ==============================================================================
        if valid_len > 0:
            ...
            # --- Label: Event (适配 30min) ---
            evt_thresh = (curr_vol * 6.0).clip(min=0.005) 
            ...
            # =================================================================
            # 4. [The Ultimate Holy Grail] Regime-Normalized Gamma Label
            # =================================================================
            
            # 提取成交量加权真实定价 IV (防类型降级护甲)
            if 'options_vw_iv' in df.columns:
                current_iv = df['options_vw_iv']
            else:
                opt_7 = df['opt_7'] if 'opt_7' in df.columns else pd.Series(np.nan, index=df.index)
                opt_15 = df['opt_15'] if 'opt_15' in df.columns else pd.Series(np.nan, index=df.index)
                current_iv = (opt_7 + opt_15) / 2.0
            
            if not isinstance(current_iv, pd.Series):
                current_iv = pd.Series(current_iv, index=df.index)
                
            safe_iv = current_iv.replace(0, np.nan).ffill().fillna(0.20)
            
            # 构造日内 U 型波动率折算因子 (Time-of-Day Multiplier)
            time_float = df['timestamp'].dt.hour + df['timestamp'].dt.minute / 60.0
            
            u_conditions = [
                (time_float >= 9.5) & (time_float < 10.0),   
                (time_float >= 10.0) & (time_float < 11.0),  
                (time_float >= 11.0) & (time_float < 13.5),  
                (time_float >= 13.5) & (time_float < 15.0),  
                (time_float >= 15.0) & (time_float <= 16.0)  
            ]
            u_choices = [1.6, 1.2, 0.7, 0.9, 1.4]
            u_shape_factor = np.select(u_conditions, u_choices, default=1.0)
            
            # 计算 Expected Move
            expected_move_30m = safe_iv * np.sqrt(k_slow / (252.0 * 390.0)) * u_shape_factor
            
            # 计算 Gap-Forward TWAP Return (t+2 to t+31 相对于 t+1 的均价收益)
            simulated_entry = df[price_col].shift(-1)
            forward_twap = df[price_col].rolling(window=k_slow).mean().shift(-(k_slow + 1))
            smoothed_return_fwd = (forward_twap / simulated_entry - 1.0).fillna(0.0)
            
            # 盘口摩擦 Cost (过滤死水震荡)
            cost_friction = 0.0005 
            effective_return = np.sign(smoothed_return_fwd) * np.maximum(0, np.abs(smoothed_return_fwd) - cost_friction)
            
            # 状态归一化偏移 (Norm Move)
            norm_move = effective_return / expected_move_30m
            
            # 引入 Gamma Convexity (1.5次方凸性放大)
            gamma_label = np.sign(norm_move) * np.power(np.abs(norm_move), 1.5)
            
            # 黑天鹅极端值截断保护
            df['label_return_fwd'] = gamma_label.fillna(0.0).clip(lower=-20.0, upper=20.0)
            # =================================================================
        else:
            df['label_direction'] = 1
            df['label_event'] = 0
...
def process_labels_file(file_path: Path, config: dict = None) -> dict:
    """
    [New Delta 架构 - 5min 核心实战版 + Cost-Penalized Vol-Scaled Target]
    修改点:
    1. k_slow: 30 -> 5 (降维至 5 分钟，对抗 VIX Spike 的物理低通滤波器)
    2. Target: 彻底切换为 (Return - Cost) / Vol，强制模型忽略不能变现的噪音。
    """
    try:
        # 1. [I/O] 读取数据
        if not file_path.exists():
        ...
        # [修改点 A] 核心预测窗口 (Horizon 调优至 5 min)
        k_slow = 30
        ...
        if valid_len > 0:
           ...
           # =================================================================
            # [The Holy Grail] 期权圣杯标签：Realized TWAP vs Implied Move
            # =================================================================
            
            k_slow = 30
            
            # 1. 提取预计算好的成交量加权真实定价 IV (options_vw_iv)
            if 'options_vw_iv' in df.columns:
                current_iv = df['options_vw_iv']
            else:
                # 容错：安全地获取 Series，如果列不存在，则生成全为 NaN 的 Series
                opt_7 = df['opt_7'] if 'opt_7' in df.columns else pd.Series(np.nan, index=df.index)
                opt_15 = df['opt_15'] if 'opt_15' in df.columns else pd.Series(np.nan, index=df.index)
                current_iv = (opt_7 + opt_15) / 2.0
            
            # 强制确保它是 Series（防止发生意外的标量降级）
            if not isinstance(current_iv, pd.Series):
                current_iv = pd.Series(current_iv, index=df.index)
            
            # 防止 IV 数据缺失或为 0 导致除以零，给一个兜底的默认 IV (比如 20% 年化)
            # 使用 .ffill() 替代 method='ffill' 以兼容最新版的 Pandas
            safe_iv = current_iv.replace(0, np.nan).ffill().fillna(0.20)
            
            # 2. 计算 Expected Move (把年化 IV 转化为未来 30 分钟的预期振幅)
            # 一年有 252 个交易日，每天 390 分钟交易时间
            # 公式: Expected Move = IV * sqrt(t / T)
            expected_move_30m = safe_iv * np.sqrt(k_slow / (252.0 * 390.0))
            
            # 3. 计算 Realized Move (保留 Gap-Forward TWAP 防实盘延迟)
            simulated_entry = df[price_col].shift(-1)
            forward_twap = df[price_col].rolling(window=k_slow).mean().shift(-(k_slow + 1))
            smoothed_return_fwd = (forward_twap / simulated_entry - 1.0).fillna(0.0)
            
            # 4. 引入期权特有的盘口摩擦 Cost (过滤没法变现的毛刺)
            cost_friction = 0.0005 
            effective_return = np.sign(smoothed_return_fwd) * np.maximum(0, np.abs(smoothed_return_fwd) - cost_friction)
            
            # 5. [终极封神 Target]
            # 物理意义: 未来 30 分钟的真实去摩擦趋势，是做市商基于真实成交量定价预期振幅的多少倍！
            # 如果 Label > 1.0，说明这是一个被市场严重低估的爆炸性机会！
            df['label_return_fwd'] = (effective_return / expected_move_30m).fillna(0.0)
            
            # 截断保护，防止模型被极端末日妖股的单一数据点破坏梯度
            df['label_return_fwd'] = df['label_return_fwd'].clip(lower=-15.0, upper=15.0)
            # =================================================================
        else:
            df['label_direction'] = 1
            df['label_event'] = 0
...
def process_option_labels_file_vectorized(feature_file_path: Path, config: dict):
    ...
    try:
        ...
        if 't0_gamma' in feature_df.columns and 't0_theta' in feature_df.columns:
            ...
            # 策略：如果 mid_price < 0.10，视为垃圾期权，标签强制为 0
            # 否则计算 Total PnL / Mid Price
            safe_mid = t0_mid.where(t0_mid > 0.10, np.nan) # 小于0.1变为NaN
            feature_df['label_gamma_pnl_std'] = total_pnl / safe_mid
            feature_df['label_gamma_pnl_std'] = feature_df['label_gamma_pnl_std'].fillna(0.0)
...
def main():
    ...
    date_range = pd.date_range(start="2022-03-01", end="2025-12-31", freq='MS')
...
def process_atm_iv_file(feature_file_path: Path):
    """
    [修复版 V6 - 删除无效时间 + 线性插值平滑]
    1. 清洗：强制删除 High Features 中时间戳为空的行（技术必须）。
    2. 合并：使用 merge_asof 对齐。
    3. 平滑：合并后使用 interpolate() 进行线性插值，利用数据的连续性填补空缺。
    """
    ny_tz = 'America/New_York'
    
    column_mapping = {
        'atm_iv': 'options_struc_atm_iv',
        'iv_skew': 'options_struc_skew',
        'term_structure': 'options_struc_term'
    }

    path_str = str(feature_file_path)

    try:
        # --- 1. 读取股票特征数据 ---
        feature_df = pd.read_parquet(feature_file_path)
        if feature_df.empty:
            return {"status": "skipped", "path": path_str, "message": "股票特征文件为空"}
        ...
        feature_df.dropna(subset=['timestamp'], inplace=True)
        
        if feature_df['timestamp'].dt.tz is None:
            feature_df['timestamp'] = feature_df['timestamp'].dt.tz_localize(ny_tz, ambiguous='infer')
        else:
            feature_df['timestamp'] = feature_df['timestamp'].dt.tz_convert(ny_tz)
            
        feature_df = feature_df.sort_values('timestamp')

        # --- 2. 定位 High Features 文件 ---
        stock_symbol = feature_file_path.parts[-5]
        year_month = feature_file_path.stem
        high_features_path = OPTION_MONTHLY_DIR / stock_symbol / 'high_features' / f"{year_month}.parquet"

        if not high_features_path.exists():
            return {"status": "skipped", "path": path_str, "message": f"高级特征文件未找到: {high_features_path}"}

        # --- 3. 读取并处理期权特征 ---
        high_features_df = pd.read_parquet(high_features_path)
        if high_features_df.empty:
            return {"status": "skipped", "path": path_str, "message": "期权高级特征文件为空"}

        # 统一时间列名
        if 'timestamp' not in high_features_df.columns:
            for col in ['datetime', 'ts', 'date']:
                if col in high_features_df.columns:
                    high_features_df.rename(columns={col: 'timestamp'}, inplace=True)
                    break
        
        if 'timestamp' not in high_features_df.columns:
            return {"status": "error", "path": path_str, "message": "High Features 缺少时间戳"}

        # [关键步骤] 清洗右表时间
        if not pd.api.types.is_datetime64_any_dtype(high_features_df['timestamp']):
            high_features_df['timestamp'] = pd.to_datetime(high_features_df['timestamp'], errors='coerce')

        # >>> 必须删除 NaT 时间，否则 merge_asof 会报错 <<<
        # 我们删除了这行数据，但依靠数据的平滑性，在后续合并中弥补
        high_features_df.dropna(subset=['timestamp'], inplace=True)
        
        if high_features_df.empty:
             return {"status": "skipped", "path": path_str, "message": "High Features 清洗后无有效时间数据"}

        # 统一时区
        if high_features_df['timestamp'].dt.tz is None:
            high_features_df['timestamp'] = high_features_df['timestamp'].dt.tz_localize(ny_tz, ambiguous='infer')
        else:
            high_features_df['timestamp'] = high_features_df['timestamp'].dt.tz_convert(ny_tz)
        
        high_features_df = high_features_df.sort_values('timestamp')

        # --- 4. 应用 Column Mapping ---
        valid_rename_map = {k: v for k, v in column_mapping.items() if k in high_features_df.columns}
        
        if not valid_rename_map:
            return {"status": "skipped", "path": path_str, "message": "High Features 中未找到指定的 mapping 列"}

        high_features_df.rename(columns=valid_rename_map, inplace=True)
        cols_to_merge = list(valid_rename_map.values())

        # 准备右表
        right_df = high_features_df[['timestamp'] + cols_to_merge].copy()

        # --- 5. 合并数据 ---
        # 先清理同名列
        feature_df.drop(columns=[c for c in cols_to_merge if c in feature_df.columns], inplace=True, errors='ignore')

        # 使用 merge_asof 进行对齐
        # 注意：这里 tolerance 设大一点（比如 30分钟），给插值留出空间
        # 如果中间缺了10分钟数据，我们希望能匹配到上一个，然后让 interpolate 去平滑过渡
        merged_df = pd.merge_asof(
            feature_df,
            right_df,
            on='timestamp',
        ...
        # 步骤 A: 将 merge 没匹配上的(超过 tolerance)设为 NaN，而不是 0
        # (merge_asof 默认如果匹配不到产生 NaN，这很好)
        
        # 步骤 B: 对时间序列进行插值 (利用 IV 的平滑性)
        # limit_direction='both' 确保开头和结尾也能填上
        # method='linear' 线性插值，适合平滑曲线
        merged_df[cols_to_merge] = merged_df[cols_to_merge].interpolate(method='linear', limit_direction='both')
        
        # 步骤 C: 兜底填充 (防止插值后仍有极少量的 NaN)
        merged_df[cols_to_merge] = merged_df[cols_to_merge].fillna(0.0)

        merged_df.to_parquet(feature_file_path, index=False, compression='zstd', compression_level=9)

        return {
            "status": "success", 
            "path": path_str, 
            "merged_columns": cols_to_merge
        }

    except Exception as e:
        import traceback
        return {"status": "error", "path": path_str, "message": f"{e}\n{traceback.format_exc()}"}

def update_atm_iv_in_files(config: dict):
    """
    【新】管理者函数：批量从 high_features 更新 atm_iv 等指标。
    """
    # 扫描所有特征文件
    tasks = list(OUTPUT_FEATURES_DIR.glob('*/*/*/*/*.parquet'))
    if not tasks:
    ...
    logging.info(f"找到 {len(tasks)} 个文件，开始同步更新 ATM IV (High Features)...")

    # 我们不需要 config 对象传递给 worker，因为逻辑是通用的
    # 直接使用 ProcessPoolExecutor
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=MAX_WORKERS) as executor:
        # map 直接返回结果迭代器
        results = list(tqdm(executor.map(process_atm_iv_file, tasks), total=len(tasks), desc="Updating ATM IV"))
    ...
    if failures:
        log_file = "atm_iv_update_failures.log"
        with open(log_file, "w", encoding='utf-8') as f:
            for fail in failures:
                f.write(f"File: {fail['path']}\nError: {fail['message']}\n{'-'*50}\n")
        print(f"\n⚠️ 有 {len(failures)} 个文件处理失败，详情见 {log_file}")
    
    logging.info(f"ATM IV 更新完成: 成功 {success_count}, 跳过 {skipped_count}, 失败 {len(failures)}")
...
def update_calculate_ker():
    """
    【新】管理者函数：批量更新所有特征文件中的 'ker' 特征。
    """
    # 扫描所有特征文件
    tasks = list(OUTPUT_FEATURES_DIR.glob('*/*/*/*/*.parquet'))
    if not tasks:
    ...
    logging.info(f"找到 {len(tasks)} 个文件，开始批量更新 'KER (Kaufman Efficiency Ratio)'...")
    ...
    # 使用多进程并行处理
    with concurrent.futures.ProcessPoolExecutor(max_workers=MAX_WORKERS) as executor:
        results = list(tqdm(executor.map(process_ker_file, tasks), total=len(tasks), desc="Updating KER"))
    ...
    # 记录失败日志
    if failures:
        log_file = "ker_update_failures.log"
        with open(log_file, "w", encoding='utf-8') as f:
            for fail in failures:
                f.write(f"File: {fail['path']}\nError: {fail['message']}\n{'-'*50}\n")
        print(f"\n⚠️ 有 {len(failures)} 个文件处理失败，详情见 {log_file}")
    
    logging.info(f"KER 更新完成: 成功 {success_count}, 跳过 {skipped_count}, 失败 {len(failures)}")
