...
LOG="$REPO/qqq_btc/results/rebuild_train_v4_dte2_ladder.log"
MANIFEST="$HOME/train_data/builds/v4_dte2_ladder/manifest.json"
CKPT_DIR="$REPO/checkpoints_qqq_v4_dte2_ladder"
LMDB_ROOT="$HOME/train_data/lmdb"
EVAL_OUT="/tmp/qqq_btc_test_eval_v4_dte2_ladder"
...
TEST_END="2026-04-30"
FEATURE_CFG="$REPO/qqq_btc/CONFIG/slow_feature_qqq_v4_dte2_ladder.json"
SYM_MAP="$REPO/qqq_btc/CONFIG/symbol_map.json"
ANCHOR_CFG="$REPO/qqq_btc/CONFIG/anchor_qqq_2dte_ladder.json"
...
RAW_FEAT="$HOME/train_data/quote_features_raw_dte2_ladder"
TRAIN_FEAT="$HOME/train_data/quote_features_train_dte2_ladder"
VAL_FEAT="$HOME/train_data/quote_features_val_dte2_ladder"
TEST_FEAT="$HOME/train_data/quote_features_test_dte2_ladder"
...
log "=== V4 dte2_ladder full rebuild start ==="
...
log "=== [0/11] step3: dte2_ladder 1s → 1m (--force) ==="
...
log "=== purge dte2_ladder pipeline outputs ==="
for sub in \
  "quote_options_day_iv_dte2_ladder/QQQ" \
  "quote_options_monthly_iv_dte2_ladder/QQQ" \
  "quote_options_bucketed_v7_dte2_ladder/QQQ" \
  "quote_features_raw_dte2_ladder/QQQ" \
  "quote_features_train_dte2_ladder/QQQ" \
  "quote_features_val_dte2_ladder/QQQ" \
  "quote_features_test_dte2_ladder/QQQ"; do
  rm -rf "$HOME/train_data/${sub}"
done
rm -rf "$EVAL_OUT" "$CKPT_DIR" \
  "$LMDB_ROOT/train_qqq_v4_dte2_ladder.lmdb" \
  "$LMDB_ROOT/val_qqq_v4_dte2_ladder.lmdb" \
  "$LMDB_ROOT/test_qqq_v4_dte2_ladder.lmdb"
mkdir -p "$CKPT_DIR"
...
with ProcessPoolExecutor(max_workers=32) as ex:
    list(tqdm(ex.map(worker, tasks), total=len(tasks), desc="split dte2_ladder"))
...
for stage in ["train_dte2_ladder", "val_dte2_ladder", "test_dte2_ladder"]:
...
print("rolling norm dte2_ladder done")
PY

log "=== [7/11] label_pipeline dynamic ladder ==="
for stage in train_dte2_ladder val_dte2_ladder test_dte2_ladder; do
  ...
  "$PY" qqq_btc/tools/label_pipeline.py \
    ...
    --report "/tmp/label_report_v4_dte2_ladder_${stage}.json"
done
...
import json
for stage in ("train_dte2_ladder", "val_dte2_ladder", "test_dte2_ladder"):
    rep = json.load(open(f"/tmp/label_report_v4_dte2_ladder_{stage}.json"))
...
for stage in train val test; do
  "$PY" qqq_btc/tools/build_lmdb.py \
    --feature-root "$HOME/train_data/quote_features_${stage}_dte2_ladder" \
    --config "$FEATURE_CFG" \
    --symbol-map "$SYM_MAP" \
    --output "$LMDB_ROOT/${stage}_qqq_v4_dte2_ladder.lmdb" \
    --symbols QQQ \
    --window-step 1
done

log "=== [9/11] pretrain V4 dte2_ladder ==="
"$PY" -m qqq_btc.model.train \
  --mode pretrain \
  --config "$FEATURE_CFG" \
  --data-root "$LMDB_ROOT" \
  --train-lmdb train_qqq_v4_dte2_ladder.lmdb \
  --val-lmdbs val_qqq_v4_dte2_ladder.lmdb \
...
manifest = {
    ...
    "note": "strict 2DTE 8-bucket ladder polygon quotes; dynamic best executable label; stock source unchanged",
    "paths": {
        "option_1s": "$OPTION_1S",
        "option_1m": "$OPTION_1M",
        "stock": "$STOCK_ROOT",
