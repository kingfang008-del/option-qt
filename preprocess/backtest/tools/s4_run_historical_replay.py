...
async def main():
    ...
    for ts_val, group in tqdm(grouped, desc="Replaying", total=unique_groups):
        ...
        packet = {
            ...
            'event_prob': group['event_prob'].fillna(0.0).values.astype(np.float32) if 'event_prob' in group.columns else np.zeros(len(group), dtype=np.float32),
