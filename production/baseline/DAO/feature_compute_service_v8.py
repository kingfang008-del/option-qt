...
class FeatureComputeService:
    ...
    async def process_market_data(self, batch, current_replay_ts=None, return_payload=False):
        ...
        try:
            for payload in batch:
                ...
                if sym:
                    # 🚀 [终极兼容修复] 必须优先取非空的那个，防止因初始化时的空 [] 导致遮蔽了真实数据
                    b_data = payload.get('buckets')
                    if not b_data or len(b_data) == 0:
                        b_data = payload.get('option_buckets')
                    
                    if b_data:
                        self.latest_opt_buckets[sym] = b_data
                        
                    c_data = payload.get('contracts')
                    if not c_data or len(c_data) == 0:
                        c_data = payload.get('option_contracts')
                        
                    if c_data:
                        self.latest_opt_contracts[sym] = c_data
    ...
    async def run_compute_cycle(self, ts_from_payload=None, return_payload=False):
        ...
        if int(sync_ts) % 10 == 0: # 提高频率到 10s 一次
            logger.info(f"🕒 [Time-Sync] Tick:{datetime.fromtimestamp(sync_ts, NY_TZ).strftime('%H:%M:%S')} | Alpha_Log_TS:{datetime.fromtimestamp(data_ts, NY_TZ).strftime('%H:%M:%S')} | Symbols:{len(ready_symbols)}")
