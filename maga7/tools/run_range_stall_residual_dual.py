#!/usr/bin/env python3
"""Dual-window residual range_stall arms for 02-06 / 03-18.

Baseline OFF = current wired spine gate (RS90_FFO2_P5 + peer_pre5≤3).

Arms:
  FFO012     — lower chase-arm min_fav_from_open 2%→1.2% (03-18 AMZN ffo≈1.41%)
  CROWD7     — peer≥7 & chase≥0.9 & pre5≤10bp & ffo≥2% (02-06 TSLA / 02-25 MSFT)
  FFO012_C7  — both
"""
...
FOCUS = ["2026-02-06", "2026-02-25", "2026-03-18", "2026-04-15", "2026-06-10", "2026-06-11"]


def _cur(**kw: Any) -> dict[str, Any]:
    cfg = {
        "enabled": True,
        "mode": "block",
        "min_chase": 0.9,
...
ARMS = {
    "OFF": None,  # profile as-is
    "FFO012": _cur(min_fav_from_open=0.012),
    "CROWD7": _cur(crowd_min_peer=7, crowd_max_pre5=0.001),
    "FFO012_C7": _cur(
        min_fav_from_open=0.012, crowd_min_peer=7, crowd_max_pre5=0.001
    ),
}
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--out", default="maga7/results/range_stall_residual_dual_v1")
    ...
    for arm in [a.strip() for a in args.arms.split(",") if a.strip()]:
        ...
        for wname, start, end in WINDOWS:
            ...
            (sub / "focus.json").write_text(json.dumps(focus_tr, indent=2), encoding="utf-8")
            r = {
                "arm": arm,
                "window": wname,
            ...
            print(
                ...
                f"blk={r['n_range_stall_block']} focus_sum={r['focus_day_ret_sum']}",
                flush=True,
            )
    ...
    summary = {
        "verdict": verdict,
        "best": best,
        "rule": "FFO012 and/or CROWD7 on top of wired range_stall",
