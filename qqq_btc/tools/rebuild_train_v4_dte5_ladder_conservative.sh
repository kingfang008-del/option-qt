#!/usr/bin/env bash
# V4 全链路重建：Polygon dte5_options 8-bucket ladder 1s → 1m → dynamic value-score label → LMDB → pretrain
#
# 数据切分:
#   train 2022-04 ~ 2025-12
#   val   2026-01 ~ 2026-03
#   test  2026-04 ~ 2026-06
# 股票数据源: spnq_train_resampled（不变）
# anchor: qqq_5dte_ladder_conservative (5DTE 8-bucket dynamic value-score label)
set -euo pipefail

REPO="/home/kingfang007/文档/GitHub/option-qt"
PY="/home/kingfang007/anaconda3/envs/ibkr/bin/python"
LOG="$REPO/qqq_btc/results/rebuild_train_v4_dte5_ladder_conservative.log"
MANIFEST="$HOME/train_data/builds/v4_dte5_ladder_conservative/manifest.json"
CKPT_DIR="$REPO/checkpoints_qqq_v4_dte5_ladder_conservative"
LMDB_ROOT="$HOME/train_data/lmdb"
EVAL_OUT="/tmp/qqq_btc_test_eval_v4_dte5_ladder_conservative"
...
TEST_END="2026-06-30"
FEATURE_CFG="$REPO/qqq_btc/CONFIG/slow_feature_qqq_v4_dte5_ladder_conservative.json"
SYM_MAP="$REPO/qqq_btc/CONFIG/symbol_map.json"
ANCHOR_CFG="$REPO/qqq_btc/CONFIG/anchor_qqq_5dte_ladder_conservative.json"
OPTION_1S="/mnt/s990/data/raw_1s/dte5_options"
OPTION_1M="/mnt/s990/data/raw_1m/dte5_options"
DAY_IV="$HOME/train_data/quote_options_day_iv_dte5_ladder"
MONTHLY_IV="$HOME/train_data/quote_options_monthly_iv_dte5_ladder"
BUCKETED="$HOME/train_data/quote_options_bucketed_v7_dte5_ladder"
RAW_FEAT="$HOME/train_data/quote_features_raw_dte5_ladder_conservative"
TRAIN_FEAT="$HOME/train_data/quote_features_train_dte5_ladder_conservative"
VAL_FEAT="$HOME/train_data/quote_features_val_dte5_ladder_conservative"
TEST_FEAT="$HOME/train_data/quote_features_test_dte5_ladder_conservative"

export PYTHONPATH="$REPO${PYTHONPATH:+:$PYTHONPATH}"
export FEATURE_CONFIG="$FEATURE_CFG"
export LOCKED_TARGETS_MAP="$HOME/train_data/locked_targets_map_5dte_ladder_202204_202606.parquet"
...
log "=== V4 dte5_ladder_conservative full rebuild start ==="
...
log "=== [0/11] step3: dte5_ladder_conservative 1s → 1m (--force) ==="
...
log "=== purge dte5_ladder_conservative pipeline outputs ==="
for sub in \
  "quote_features_raw_dte5_ladder_conservative/QQQ" \
  "quote_features_train_dte5_ladder_conservative/QQQ" \
  "quote_features_val_dte5_ladder_conservative/QQQ" \
  "quote_features_test_dte5_ladder_conservative/QQQ"; do
  rm -rf "$HOME/train_data/${sub}"
done
rm -rf "$EVAL_OUT" "$CKPT_DIR" \
  "$LMDB_ROOT/train_qqq_v4_dte5_ladder_conservative.lmdb" \
  "$LMDB_ROOT/val_qqq_v4_dte5_ladder_conservative.lmdb" \
  "$LMDB_ROOT/test_qqq_v4_dte5_ladder_conservative.lmdb"
mkdir -p "$CKPT_DIR"
...
with ProcessPoolExecutor(max_workers=32) as ex:
    list(tqdm(ex.map(worker, tasks), total=len(tasks), desc="split dte5_ladder_conservative"))
...
for stage in ["train_dte5_ladder_conservative", "val_dte5_ladder_conservative", "test_dte5_ladder_conservative"]:
...
print("rolling norm dte5_ladder_conservative done")
PY

log "=== [7/11] label_pipeline dynamic ladder ==="
for stage in train_dte5_ladder_conservative val_dte5_ladder_conservative test_dte5_ladder_conservative; do
  ...
  "$PY" qqq_btc/tools/label_pipeline.py \
    ...
    --report "/tmp/label_report_v4_dte5_ladder_conservative_${stage}.json"
done
...
import json
for stage in ("train_dte5_ladder_conservative", "val_dte5_ladder_conservative", "test_dte5_ladder_conservative"):
    rep = json.load(open(f"/tmp/label_report_v4_dte5_ladder_conservative_{stage}.json"))
...
for stage in train val test; do
  "$PY" qqq_btc/tools/build_lmdb.py \
    --feature-root "$HOME/train_data/quote_features_${stage}_dte5_ladder_conservative" \
    --config "$FEATURE_CFG" \
    --symbol-map "$SYM_MAP" \
    --output "$LMDB_ROOT/${stage}_qqq_v4_dte5_ladder_conservative.lmdb" \
    --symbols QQQ \
    --window-step 1
done

log "=== [9/11] pretrain V4 dte5_ladder_conservative ==="
"$PY" -m qqq_btc.model.train \
  --mode pretrain \
  --config "$FEATURE_CFG" \
  --data-root "$LMDB_ROOT" \
  --train-lmdb train_qqq_v4_dte5_ladder_conservative.lmdb \
  --val-lmdbs val_qqq_v4_dte5_ladder_conservative.lmdb \
...
manifest = {
    "build_id": "v4_dte5_ladder_conservative",
    ...
    "note": "strict 5DTE 8-bucket ladder polygon quotes; conservative entry-time value-score label; stock source unchanged",
    "paths": {
        "option_1s": "$OPTION_1S",
        "option_1m": "$OPTION_1M",
        "stock": "$STOCK_ROOT",
...
log "=== V4 dte5_ladder_conservative DONE ==="
