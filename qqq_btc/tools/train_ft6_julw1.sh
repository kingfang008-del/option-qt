#!/usr/bin/env bash
# V4 在 bak 仅 6 月上 finetune（对照 FT56=5–6），再 replay 2026-07 W1
# train=Jun, val=Jun, init=V4, seed=42
set -euo pipefail
...
CKPT_OUT="${CKPT_OUT:-checkpoint/checkpoints_qqq_ft6_julw1}"
EVAL_FT="${EVAL_FT:-qqq_btc/results/ft6_julw1_fixed5m_infer}"
RESULTS="${RESULTS:-qqq_btc/results/ft6_julw1_compare}"
...
TRAIN_FEAT="/tmp/quote_features_ft6_train"
VAL_FEAT="/tmp/quote_features_ft6_val"
LMDB_TRAIN="${LMDB_TRAIN:-train_qqq_ft6.lmdb}"
LMDB_VAL="${LMDB_VAL:-val_qqq_ft6.lmdb}"
...
echo "=== [1] feature dirs: train=6, val=6 ==="
setup_months "$TRAIN_FEAT" 2026-06
...
OUT = Path("qqq_btc/results/ft6_julw1_compare")
...
models = {
    "ft6": "qqq_btc/results/ft6_julw1_fixed5m_infer/test_infer.parquet",
...
for name, path in models.items():
    ...
    rows.append(row)
    if name == "ft6":
        trades.to_parquet(OUT / "ft6_replay_trades.parquet", index=False)
        detail = row

out = {
    "recipe": "fixed5m edge + 1min_causal(+1m) put_gate + LIVE + q10=-0.2",
    "ft6": {"train": "2026-06 bak", "val": "2026-06 bak", "init": "checkpoints_qqq_v4", "ckpt": "checkpoint/checkpoints_qqq_ft6_julw1/best.pth"},
