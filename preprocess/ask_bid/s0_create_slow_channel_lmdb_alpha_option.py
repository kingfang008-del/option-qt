...
描述: [v3.0 New York Timezone 版 - 慢通道]
      ...
      4. [新增] 全面支持期权定价偏差(Pricing Alpha)新标签。
"""
        ...
        for (session_name, time_range, res), file_list in files_by_group.items():
            ...
            for f_1m in file_list:
                ...
                try:
                    ...
                    # 🚨 [核心修复 2] 更安全的 np 数组转换，防止 vol_direction 默认为 0
                    def to_np(df, cols):
                        for c in cols: 
                            if c not in df.columns: 
                                if c == 'label_vol_direction':
                                    df[c] = 1.0  # 方向默认 1 (Flat)
                                else:
                                    df[c] = 0.0
...
def verify_data_integrity(lmdb_path, config, parquet_root):
    ...
    try:
        ...
        with env.begin() as txn:
            ...
            if not keys_c: 
                print("❌ 致命错误: 未在 LMDB 中找到 __keys__！文件可能损坏或完全没写进去。")
                return
            ...
            if total_samples == 0:
                print("\n" + "!"*60)
                print("🚨🚨🚨 极其致命的错误 (FATAL ERROR) 🚨🚨🚨")
                print(f"LMDB 文件 {lmdb_path.name} 已生成，但里面没有任何数据 (0 samples)！")
                print("可能的原因：")
                print("  1. 你的 Parquet 源数据时间范围与 STAGES_TO_PROCESS 的切分日期不匹配。")
                print("  2. 源数据在这个时间段内全部被死水/无量过滤逻辑清除了。")
                print("  3. 源文件路径不正确。")
                print("请立即检查脚本顶部的 STAGES_TO_PROCESS 字典的日期范围！")
                print("!"*60 + "\n")
                return
            
            print(f"✅ 样本量检查通过: 成功在 LMDB 中找到 {total_samples} 条数据！")
            
            # 随机抽样
            sample_keys = random.sample(all_keys, min(3, total_samples))
            for key_bytes in sample_keys:
                ...
                # 🚨 打印 Labels 的内容，确认已成功打包
                data_lbl = sample_data.get('labels', {})
                print(f"✅ [Labels] 标签写入抽样:")
                print(f"    - Gamma PnL Std:  {data_lbl.get('label_gamma_pnl_std', 0):.6f}")
                print(f"    - IV-RV Spread:   {data_lbl.get('label_iv_rv_spread', 0):.6f}")
                print(f"    - Vol Direction:  {data_lbl.get('label_vol_direction', 1)}")
