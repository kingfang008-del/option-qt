#!/usr/bin/env python3
"""SMC/ICT-lite + order-flow proxy scout (DN) on stock 1s; option trades TP/SL.

Independent of Mag7 Rule-A / Hunt. No baseline-miss filter.

Morphs (1s OHLCV proxies — no aggressor tape):
  - sweep_rev_dn: take out prior swing high then close back below + displacement down
  - bos_disp_dn:  close breaks prior swing low + displacement down
Flow confirms (ablation):
  - off | dn_vol_share≥τ | mf<0 & streak_dn≥k

Dual windows: may_jul09 / jul10_23.

Example:
  PYTHONPATH=. python -m maga7.tools.scan_smc_flow_scout_tpsl \\
    --tag research_smc_flow_scout_dn_tpsl_dual
"""
...
FLOW_MODES = (
    ("off", None, 0, False),
    ("share55", 0.55, 0, False),
    ("share60", 0.60, 0, False),
    ("mf_st5", None, 5, True),
)
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--tag", default="research_smc_flow_scout_dn_tpsl_dual")
    ap.add_argument("--trades-root", default=str(DEFAULT_TRADES))
    ap.add_argument(
        "--sessions",
        default="AM_0935_1030,CORE_1030_1200,MID_1200_1400,PM_1400_1530",
    )
    ap.add_argument("--morphs", default="sweep_rev_dn,bos_disp_dn")
    ap.add_argument("--swing-sec", default="180,300")
    ap.add_argument("--disp-sec", type=int, default=60)
    ap.add_argument("--disp-thr", default="0.003,0.005,0.008")
    ap.add_argument("--flow-sec", type=int, default=120)
    ap.add_argument(
        "--flow-modes",
        default="off,share55,share60,mf_st5",
        help="Subset of off|share55|share60|mf_st5",
    )
    ...
    swings = [int(x) for x in args.swing_sec.split(",") if x.strip()]
    thrs = [float(x) for x in args.disp_thr.split(",") if x.strip()]
    tps = [float(x) for x in args.tp.split(",") if x.strip()]
    sls = [float(x) for x in args.sl.split(",") if x.strip()]
    want_flow = {x.strip() for x in args.flow_modes.split(",") if x.strip()}
    flow_modes = [m for m in FLOW_MODES if m[0] in want_flow]
    ...
    print(
        f"smc_flow DN scout {start_all}..{end_all} days={len(dates)} "
        f"morphs={morphs} flow={[m[0] for m in flow_modes]}",
        flush=True,
    )

    # Arm key params (excl tp/sl)
    arm_specs: list[dict[str, Any]] = []
    for morph in morphs:
        for swing in swings:
            for thr in thrs:
                for fname, share, streak, mf_neg in flow_modes:
                    arm_specs.append(
                        {
                            "morph": morph,
                            "swing_sec": int(swing),
                            ...
                            "disp_thr": float(thr),
                            "flow_sec": int(args.flow_sec),
                            "flow_mode": fname,
                            "min_dn_vol_share": share,
                            "min_streak_dn": int(streak),
                            "require_mf_neg": bool(mf_neg),
                            "spec_key": (
                                f"{morph}_sw{swing}_d{thr}_"
                                f"f{fname}"
                            ),
                        }
                    )
    ...
    for di, date in enumerate(dates):
        ...
        for sym in symbols:
            ...
            for sess_name, s0, s1 in sessions:
                warm = max(int(args.disp_sec), max(swings), int(args.flow_sec))
                ...
                for spec in arm_specs:
                    hit = first_smc_flow_dn_in_window(
                        ...
                        t_start=t_start,
                        t_end=t_end,
                        morph=str(spec["morph"]),
                        swing_sec=int(spec["swing_sec"]),
                        disp_sec=int(spec["disp_sec"]),
                        disp_thr=float(spec["disp_thr"]),
                        flow_sec=int(spec["flow_sec"]),
                        min_dn_vol_share=spec["min_dn_vol_share"],
                        min_streak_dn=int(spec["min_streak_dn"]),
                        require_mf_neg=bool(spec["require_mf_neg"]),
                    ...
                    arms.append(
                        {
                            ...
                            "pts": arr[0],
                            "plast": arr[1],
                            "stock_ret_disp": arm.stock_ret_disp,
                            "dn_vol_share": arm.dn_vol_share,
                            "mf": arm.mf,
                            "streak_dn": arm.streak_dn,
                            "swing_level": arm.swing_level,
                            **{k: spec[k] for k in (
                                "morph",
                                "swing_sec",
                                "disp_sec",
    ...
    summary = {
        ...
        "data_note": (
            "Structure+displacement on stock 1s OHLC; order-flow proxies = "
            "down-tick vol share + CLV sec_mf. Option trades used only for TP/SL pricing. "
            "No true aggressor / CVD / footprint."
        ),
