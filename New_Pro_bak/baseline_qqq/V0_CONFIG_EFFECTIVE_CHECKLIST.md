...
- `V0_PROFIT_HYBRID_CONTINUOUS_ENABLED`（默认 **False**，与 `config` / 环境变量一致；设 `=1` 启用 `TRAILING_EPIC`）
...
2. `spread_divergence`  
   - 策略层：空仓时 OMS 中散度为 0，入场门控主要依赖点差宽度与 bid/ask（与 S4 一致）；见 `replay_live_parity_utils` 说明与 `execution_engine_v8._build_strategy_ctx` 注释。  
...
1. 总路线图见同目录 **`S4_LIVE_PARITY_ROADMAP.md`**（S4 ↔ 实盘对齐分阶段清单）。
2. 先补 `ExecutionEngineV8._build_strategy_ctx()` 契约测试。
3. 再把“已声明但未接线”的 guard switches 做一次清理。
4. 最后统一策略层与执行层的单一规则来源，避免继续打地鼠。
