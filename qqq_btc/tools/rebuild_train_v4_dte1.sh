#!/usr/bin/env bash
# V4 全链路重建：Polygon dte1_options 1s → 1m → day_iv → feature → label → LMDB → pretrain
#
# 数据切分:
...
# anchor: qqq_1dte (strict front_dte=1)
set -euo pipefail

REPO="/home/kingfang007/文档/GitHub/option-qt"
PY="/home/kingfang007/anaconda3/envs/ibkr/bin/python"
LOG="$REPO/qqq_btc/results/rebuild_train_v4_dte1.log"
MANIFEST="$HOME/train_data/builds/v4_dte1/manifest.json"
CKPT_DIR="$REPO/checkpoints_qqq_v4_dte1"
LMDB_ROOT="$HOME/train_data/lmdb"
EVAL_OUT="/tmp/qqq_btc_test_eval_v4_dte1"
...
ANCHOR_CFG="$REPO/qqq_btc/CONFIG/anchor_qqq_1dte.json"
OPTION_1S="/mnt/s990/data/raw_1s/dte1_options"
OPTION_1M="/mnt/s990/data/raw_1m/dte1_options"
DAY_IV="$HOME/train_data/quote_options_day_iv_dte1"
MONTHLY_IV="$HOME/train_data/quote_options_monthly_iv_dte1"
BUCKETED="$HOME/train_data/quote_options_bucketed_v7_dte1"
RAW_FEAT="$HOME/train_data/quote_features_raw_dte1"
TRAIN_FEAT="$HOME/train_data/quote_features_train_dte1"
VAL_FEAT="$HOME/train_data/quote_features_val_dte1"
TEST_FEAT="$HOME/train_data/quote_features_test_dte1"

export PYTHONPATH="$REPO${PYTHONPATH:+:$PYTHONPATH}"
export FEATURE_CONFIG="$FEATURE_CFG"
export LOCKED_TARGETS_MAP="$HOME/train_data/locked_targets_map_1dte.parquet"
...
log "=== V4 dte1 full rebuild start ==="
...
log "=== [0/11] step3: dte1 1s → 1m (--force) ==="
...
log "=== purge dte1 pipeline outputs ==="
for sub in \
  "quote_options_day_iv_dte1/QQQ" \
  "quote_options_monthly_iv_dte1/QQQ" \
  "quote_options_bucketed_v7_dte1/QQQ" \
  "quote_features_raw_dte1/QQQ" \
  "quote_features_train_dte1/QQQ" \
  "quote_features_val_dte1/QQQ" \
  "quote_features_test_dte1/QQQ"; do
  rm -rf "$HOME/train_data/${sub}"
done
rm -rf "$EVAL_OUT" "$CKPT_DIR" \
  "$LMDB_ROOT/train_qqq_v4_dte1.lmdb" \
  "$LMDB_ROOT/val_qqq_v4_dte1.lmdb" \
  "$LMDB_ROOT/test_qqq_v4_dte1.lmdb"
mkdir -p "$CKPT_DIR"
...
with ProcessPoolExecutor(max_workers=32) as ex:
    list(tqdm(ex.map(worker, tasks), total=len(tasks), desc="split dte1"))
...
for stage in ["train_dte1", "val_dte1", "test_dte1"]:
...
print("rolling norm dte1 done")
PY

log "=== [7/11] label_pipeline ==="
for stage in train_dte1 val_dte1 test_dte1; do
  ...
  "$PY" qqq_btc/tools/label_pipeline.py \
    ...
    --report "/tmp/label_report_v4_dte1_${stage}.json"
done
...
import json
for stage in ("train_dte1", "val_dte1", "test_dte1"):
    rep = json.load(open(f"/tmp/label_report_v4_dte1_{stage}.json"))
...
for stage in train val test; do
  "$PY" qqq_btc/tools/build_lmdb.py \
    --feature-root "$HOME/train_data/quote_features_${stage}_dte1" \
    --config "$FEATURE_CFG" \
    --symbol-map "$SYM_MAP" \
    --output "$LMDB_ROOT/${stage}_qqq_v4_dte1.lmdb" \
    --symbols QQQ \
    --window-step 1
done

log "=== [9/11] pretrain V4 dte1 ==="
...
manifest = {
    ...
    "note": "strict 1DTE polygon quotes; stock source unchanged; val=Jan-Feb / test=Mar-Apr 2026",
    "paths": {
        "option_1s": "$OPTION_1S",
        "option_1m": "$OPTION_1M",
        "stock": "$STOCK_ROOT",
