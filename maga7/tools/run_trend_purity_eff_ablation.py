#!/usr/bin/env python3
"""Ablate efficiency/noise trend-purity sizing on extend_mtm_only peer3.

``trend_purity_features=efficiency`` uses path/range/dir_frac/adverse instead of
|from_prev|+peer (which ranked Jul-09 AMD above META).
"""
...
def main() -> None:
    ...
    p.add_argument("--tag", default="trend_purity_eff_ablation_extend_mtm_peer3_may_jul")
    args = p.parse_args()
    ...
    base_purity = {
        ...
        "trend_purity_path_window": 20,
        "trend_purity_adverse_ref": 0.005,
    }

    variants: list[tuple[str, dict[str, Any], dict[str, Any]]] = [
        ("extend_mtm_only", {}, {}),
        ("mom_cont", {**base_purity, "trend_purity_features": "momentum"}, {}),
        ("eff_cont", {**base_purity, "trend_purity_features": "efficiency"}, {}),
        ("eff_cont_w30", {
            **base_purity,
            "trend_purity_features": "efficiency",
            "trend_purity_path_window": 30,
        }, {}),
        ("eff_cont_soft", {
            **base_purity,
            "trend_purity_features": "efficiency",
            "trend_purity_high": 0.55,
        ...
        ("eff_skip_low", {
            **base_purity,
            ...
            "trend_purity_skip_below": 0.35,
        }, {}),
        ("hybrid_cont", {**base_purity, "trend_purity_features": "hybrid"}, {}),
        ("confirm1_eff", {
            **base_purity,
            "trend_purity_features": "efficiency",
        ...
        ("full_day", {}, full_day),
        ("full_day_eff", {**base_purity, "trend_purity_features": "efficiency"}, full_day),
        ("full_day_confirm1_eff", {
            **base_purity,
            "trend_purity_features": "efficiency",
            "entry_confirm_bars": 1,
            "entry_confirm_mode": "mf",
    ...
    for name, trade_over, reg_over in variants:
        ...
        if len(focus):
            for r in focus.itertuples(index=False):
                ...
                pe = getattr(r, "purity_path_eff", None)
                ...
                pe_s = f" path={float(pe):.2f}" if pe is not None and pd.notna(pe) else ""
                focus_detail.append(
                    f"{r.date} {r.symbol} {r.dir} ret={float(r.ret):+.1%} "
                    f"size={float(r.size_frac):.3f}{pur_s}{psc_s}{pe_s} {r.reason}"
                )
        row = {
            "name": name,
            "features": trade_over.get("trend_purity_features"),
