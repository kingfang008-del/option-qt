...
class FeatureComputeService:
    ...
    def _step_engine_compute(self, data_ts, is_new_minute, alpha_label_ts):
        ...
        if need_full_recompute:
            ...
            try:
                from qqq_btc.live.fcs_enrich_hook import enrich_history_map

                history_1min_source = enrich_history_map(history_1min_source)
                if is_new_minute:
                    self.committed_history_1min = history_1min_source
                else:
                    self.history_1min = history_1min_source
            except ImportError:
                pass
            except Exception as e:
                logger.warning("[qqq_btc] FCS history enrich skipped: %s", e)
    ...
    def _assemble_compute_payload(
        ...
        batch_opt_spreads, batch_iv_moms = [], []
        ...
        for b_idx, sym in enumerate(self.symbols):
            ...
            idx_spread = self.feat_name_to_idx.get('options_vw_spread')
            batch_opt_spreads.append(raw_mat[b_idx, idx_spread] if idx_spread is not None else 0.0)
            idx_iv_mom = self.feat_name_to_idx.get('options_iv_momentum')
            batch_iv_moms.append(raw_mat[b_idx, idx_iv_mom] if idx_iv_mom is not None else 0.0)
        ...
        payload = {
            ...
            'options_vw_spread': np.array(batch_opt_spreads),
            'options_iv_momentum': np.array(batch_iv_moms),
...
if __name__ == "__main__":
    ...
    new_pro_cfg = REPO_ROOT / "CONFIG"
    slow_path = new_pro_cfg / "slow_feature.json"
    fast_path = new_pro_cfg / "fast_feature_qqq.json"
    slow_override = os.environ.get("SLOW_FEATURE_CONFIG", "").strip()
    if slow_override:
        slow_override_path = Path(slow_override).expanduser()
        if slow_override_path.exists():
            slow_path = slow_override_path
            logger.info("🧩 Using SLOW_FEATURE_CONFIG override: %s", slow_path)
    if not slow_path.exists():
        slow_path = home_project / "config/slow_feature.json"
    if not fast_path.exists():
        fast_path = new_pro_cfg / "fast_feature.json"
    if not fast_path.exists():
