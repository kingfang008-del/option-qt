...
def normalize_alpha_scores(
    ...
    *,
    ...
    if normalized_mode in {'absolute', 'raw', 'net_edge'}:
        pairs = _alpha_pairs(symbols, raw_alphas, exclude_symbols)
        z_by_symbol = {sym: _clip_alpha_z(val, clip) for sym, val in pairs}
        return z_by_symbol, {
            'mode': 'absolute',
            'count': len(pairs),
        }
    ...
    raise ValueError(
        ...
        "Use absolute, cross_section, bucket, rolling, or mixed."
    )
