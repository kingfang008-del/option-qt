...
from strategy.exec_profile import multi_band_roll_cooldown_seconds
import sys
...
class OrchestratorAccounting:
    ...
    def _process_exit_accounting(self, sym, st, filled_qty, fill_price, stock_price, curr_ts, reason, duration, ratio, original_position=None, execution_meta=None):
        ...
        # 若用 wall clock，speed=inf 下亏损冷却(COOLDOWN_MINUTES)会挡住当天剩余全部行情。
        ...
        # 冷却与熔断逻辑（multi_band：盈利腿短冷却滚下一档）
        cfg = getattr(getattr(self.orch, 'strategy', None), 'cfg', None)
        exec_prof = str(getattr(st, 'exec_profile', '') or '')
        if exec_prof == 'multi_band' and cfg is not None:
            st.cooldown_until = safe_now_ts + multi_band_roll_cooldown_seconds(
                cfg, profitable=(roi >= 0), reason=reason,
            )
        elif roi < 0 or "STOP" in reason or "FLIP" in reason:
