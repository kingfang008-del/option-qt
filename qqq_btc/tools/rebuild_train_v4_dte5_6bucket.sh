#!/usr/bin/env bash
# V4 全链路重建：Polygon dte5_options_6b 1s → 1m → day_iv → feature → label → LMDB → pretrain
#
# 数据切分:
#   train 2022-04 ~ 2025-12
#   val   2026-01 ~ 2026-03
#   test  2026-04 ~ 2026-06
# 6-bucket 布局与 legacy 9DTE 一致；replay 默认 call_bucket=2, put_bucket=0
set -euo pipefail

REPO="/home/kingfang007/文档/GitHub/option-qt"
PY="/home/kingfang007/anaconda3/envs/ibkr/bin/python"
LOG="$REPO/qqq_btc/results/rebuild_train_v4_dte5_6bucket.log"
MANIFEST="$HOME/train_data/builds/v4_dte5_6bucket/manifest.json"
CKPT_DIR="$REPO/checkpoints_qqq_v4_dte5_6bucket"
LMDB_ROOT="$HOME/train_data/lmdb"
EVAL_OUT="/tmp/qqq_btc_test_eval_v4_dte5_6bucket"
...
TEST_END="2026-06-30"
FEATURE_CFG="$REPO/qqq_btc/CONFIG/slow_feature_qqq_v4_dte5_6bucket.json"
SYM_MAP="$REPO/qqq_btc/CONFIG/symbol_map.json"
ANCHOR_CFG="$REPO/qqq_btc/CONFIG/anchor_qqq_5dte.json"
OPTION_1S="/mnt/s990/data/raw_1s/dte5_options_6b"
OPTION_1M="/mnt/s990/data/raw_1m/dte5_options_6b"
DAY_IV="$HOME/train_data/quote_options_day_iv_dte5_6b"
MONTHLY_IV="$HOME/train_data/quote_options_monthly_iv_dte5_6b"
BUCKETED="$HOME/train_data/quote_options_bucketed_v7_dte5_6b"
RAW_FEAT="$HOME/train_data/quote_features_raw_dte5_6b"
TRAIN_FEAT="$HOME/train_data/quote_features_train_dte5_6b"
VAL_FEAT="$HOME/train_data/quote_features_val_dte5_6b"
TEST_FEAT="$HOME/train_data/quote_features_test_dte5_6b"

export PYTHONPATH="$REPO${PYTHONPATH:+:$PYTHONPATH}"
export FEATURE_CONFIG="$FEATURE_CFG"
export LOCKED_TARGETS_MAP="$HOME/train_data/locked_targets_map_5dte_6bucket.parquet"
...
log "=== V4 dte5_6bucket full rebuild start ==="
...
log "=== [0/11] step3: dte5_6b 1s → 1m (--force) ==="
...
log "=== purge dte5_6bucket pipeline outputs ==="
for sub in \
  "quote_options_day_iv_dte5_6b/QQQ" \
  "quote_options_monthly_iv_dte5_6b/QQQ" \
  "quote_options_bucketed_v7_dte5_6b/QQQ" \
  "quote_features_raw_dte5_6b/QQQ" \
  "quote_features_train_dte5_6b/QQQ" \
  "quote_features_val_dte5_6b/QQQ" \
  "quote_features_test_dte5_6b/QQQ"; do
  rm -rf "$HOME/train_data/${sub}"
done
rm -rf "$EVAL_OUT" "$CKPT_DIR" \
  "$LMDB_ROOT/train_qqq_v4_dte5_6bucket.lmdb" \
  "$LMDB_ROOT/val_qqq_v4_dte5_6bucket.lmdb" \
  "$LMDB_ROOT/test_qqq_v4_dte5_6bucket.lmdb"
mkdir -p "$CKPT_DIR"
...
import pandas as pd
from tqdm import tqdm
from preprocess.ask_bid.split_raw_features import process_and_copy_file, get_valid_symbols
...
with ProcessPoolExecutor(max_workers=32) as ex:
    list(tqdm(ex.map(worker, tasks), total=len(tasks), desc="split dte5_6b"))
...
for stage in ["train_dte5_6b", "val_dte5_6b", "test_dte5_6b"]:
...
print("rolling norm dte5_6b done")
PY

log "=== [7/11] label_pipeline ==="
for stage in train_dte5_6b val_dte5_6b test_dte5_6b; do
  ...
  "$PY" qqq_btc/tools/label_pipeline.py \
    ...
    --report "/tmp/label_report_v4_dte5_6bucket_${stage}.json"
done
...
import json
for stage in ("train_dte5_6b", "val_dte5_6b", "test_dte5_6b"):
    rep = json.load(open(f"/tmp/label_report_v4_dte5_6bucket_{stage}.json"))
...
for stage in train val test; do
  "$PY" qqq_btc/tools/build_lmdb.py \
    --feature-root "$HOME/train_data/quote_features_${stage}_dte5_6b" \
    --config "$FEATURE_CFG" \
    --symbol-map "$SYM_MAP" \
    --output "$LMDB_ROOT/${stage}_qqq_v4_dte5_6bucket.lmdb" \
    --symbols QQQ \
    --window-step 1
done

log "=== [9/11] pretrain V4 dte5_6bucket ==="
"$PY" -m qqq_btc.model.train \
  --mode pretrain \
  --config "$FEATURE_CFG" \
  --data-root "$LMDB_ROOT" \
  --train-lmdb train_qqq_v4_dte5_6bucket.lmdb \
  --val-lmdbs val_qqq_v4_dte5_6bucket.lmdb \
...
manifest = {
    ...
    "note": "5DTE front+next 6-bucket polygon quotes; same bucket layout as legacy 9DTE; replay call=2 put=0",
    "paths": {
        "option_1s": "$OPTION_1S",
        "option_1m": "$OPTION_1M",
        "stock": "$STOCK_ROOT",
