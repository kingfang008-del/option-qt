...
st.set_page_config(page_title="QQQ 0DTE System Flow", layout="wide", page_icon="🗺️")
...
from config import (
    ...
    STRATEGY_AUTO_ENTRY_ENABLED, EXEC_PROFILE,
)
...
from dashboard_exec_topo import build_exec_topo_state, draw_exec_topo_figure
from dashboard_system_flow import build_flow_context, render_system_flow_header, render_system_flow_tab
...
# Stream 健康探测(须在主流程前定义,供系统全景 Tab 使用)
...
# --- Top: System flow summary (ARCHITECTURE 全景) ---
status = SystemStatus(r, symbol)
_eng_batch = None
if status.eng_data:
    _eng_batch = status.eng_data.get("batch_packet", status.eng_data)
_exec_topo = build_exec_topo_state(
    r,
    symbol,
    pg_url=PG_DB_URL,
    eng_batch=_eng_batch,
    src_lag=status.src_lag,
    eng_lag=status.eng_lag,
    exec_profile_env=EXEC_PROFILE,
)
_sh_rows, _sh_err = _sh_inspect_redis_groups()
_flow_ctx = build_flow_context(
    symbol=symbol,
    run_mode=RUN_MODE,
    status=status,
    exec_topo=_exec_topo,
    is_light=is_light,
    ibkr_conn=conn_status,
    stream_rows=_sh_rows,
    r=r,
)
render_system_flow_header(_flow_ctx, show_charts=True)
with st.expander("📡 数据预热拓扑 (Legacy IBKR→Stream→Engine)", expanded=False):
...
# --- Main: Tabs ---
tab_flow, tab1, tab_oms_ledger, tab10, tab2, tab11, tab3, tab4, tab5, tab6, tab7, tab8, tab9, tab_sh, tab_gates = st.tabs([
    "🗺️ 系统全景",
    "📈 Market Feed",
    "📊 OMS 持仓 / 成交",
    "🏦 IBKR Positions",
...
with tab_flow:
    _flow_ctx.stream_rows = (_sh_inspect_redis_groups()[0])
    render_system_flow_tab(_flow_ctx)
