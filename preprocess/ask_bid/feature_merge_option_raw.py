...
def _labeling_param(config: dict, name: str, default):
    """Read a labeling parameter from config, falling back to env/default."""
    env_name = f"LABEL_{name.upper()}"
    raw = os.environ.get(env_name)
    if raw is None and isinstance(config, dict):
        raw = config.get('parameters', {}).get('labeling', {}).get(name, default)
    if isinstance(default, bool):
        return str(raw).strip().lower() in {"1", "true", "yes", "on"} if raw is not None else default
    try:
...
def _estimate_round_trip_cost_series(df: pd.DataFrame, config: dict, valid_len: int) -> pd.Series:
    """
    Estimate executable friction for labels.

    If a bid/ask pair is present, approximate the round-trip mid-price penalty
    implied by BACKTEST_OPT_FILL_SPREAD_FRAC. For frac=0.75, cost ~= 0.5 * spread_pct.
    If no spread columns exist, use a configurable floor.
    """
    ...
    bid_col = _first_existing_col(df, [
        "options_bid", "option_bid", "call_bid", "atm_call_bid", "bid",
        "options_call_bid", "options_atm_call_bid",
    ])
    ask_col = _first_existing_col(df, [
        "options_ask", "option_ask", "call_ask", "atm_call_ask", "ask",
        "options_call_ask", "options_atm_call_ask",
    ])
    price_col = _first_existing_col(df, [
        "options_mid", "option_mid", "call_mid", "atm_call_mid", "mid",
        "options_price", "option_price", "call_price", "atm_call_price",
    ])

    cost = pd.Series(float(floor), index=idx, dtype="float64")
    if bid_col and ask_col:
        bid = pd.to_numeric(df.loc[idx, bid_col], errors="coerce")
        ask = pd.to_numeric(df.loc[idx, ask_col], errors="coerce")
        if price_col:
            mid = pd.to_numeric(df.loc[idx, price_col], errors="coerce")
        else:
            mid = (bid + ask) / 2.0
        spread_pct = ((ask - bid) / mid.replace(0, np.nan)).replace([np.inf, -np.inf], np.nan)
        # BUY above mid and SELL below mid: 2 * (frac - 0.5) * spread_pct.
        spread_cost = (2.0 * (fill_frac - 0.5) * spread_pct).clip(lower=0.0, upper=0.20)
        cost = pd.concat([cost, spread_cost.rename("spread_cost")], axis=1).max(axis=1).fillna(floor)
    return cost.clip(lower=0.0, upper=0.20)
...
def _apply_executable_net_labels(
    ...
    """Add cost-aware labels while preserving the old gross labels."""
    ...
    idx = df.index[:valid_len]
    gross = pd.to_numeric(gross_return.loc[idx], errors="coerce").fillna(0.0)
    cost = _estimate_round_trip_cost_series(df, config, valid_len)
    net = np.sign(gross) * np.maximum(0.0, np.abs(gross) - cost)
    flat_margin = float(_labeling_param(config, "executable_flat_margin", 0.0005))

    net_dir = np.ones(valid_len, dtype=int)
    net_arr = np.asarray(net, dtype=np.float64)
    net_dir[net_arr > flat_margin] = 2
    net_dir[net_arr < -flat_margin] = 0

    df.loc[idx, "label_return_fwd_net"] = net_arr
    df.loc[idx, "label_direction_net"] = net_dir
    df.loc[idx, "label_execution_cost"] = np.asarray(cost, dtype=np.float64)

    net_consol_ratio = float((net_dir == 1).mean() * 100.0)
    avg_cost = float(np.nanmean(np.asarray(cost, dtype=np.float64))) if len(cost) else 0.0
...
def process_labels_file(file_path: Path, config: dict = None) -> dict:
    ...
    try:
        ...
        if valid_len > 0:
            ...
            net_consol_ratio, avg_execution_cost = _apply_executable_net_labels(
                df, config or {}, gross_return, valid_len
            )
