...
try:
    sys.path.append(str(Path(__file__).resolve().parents[2] / "production" / "baseline"))
...
def resolve_config_path(path_arg=None):
    candidates = []
    if path_arg:
        candidates.append(Path(path_arg).expanduser())
    candidates.extend([
        Path(__file__).resolve().parents[1] / "CONFIG" / "slow_feature.json",
        Path(__file__).resolve().parents[1] / "config" / "slow_feature.json",
        Path.home() / "notebook/train/slow_feature.json",
    ])
    for path in candidates:
        if path.exists():
            return path
    raise FileNotFoundError(f"找不到 slow_feature.json，已尝试: {[str(p) for p in candidates]}")


def read_training_config(path_arg=None):
    config_path = resolve_config_path(path_arg)
...
def pick_device(device_arg="auto"):
    if device_arg == "auto":
...
class PerSymbolCalibrator(nn.Module):
    """
    Per-symbol affine calibration for executable edge.

    The shared TFT learns common structure, while this layer lets each symbol
    keep its own edge scale and hurdle.  This is the model-side boundary that
    prevents cross-sectional ranking from treating all option microstructures
    ...
    def forward(self, stock_id, raw_edge):
        ...
        # softplus(0) ~= 0.693, so add a small offset to keep gradients healthy.
        scale = F.softplus(self.scale(sid)) + 0.25
        bias = self.bias(sid)
        return raw_edge * scale + bias, self.context(sid)
...
class AdvancedAlphaNet(nn.Module):
    def __init__(self, config, caps, hidden_dim=64, dropout=0.3):
        ...
        self.head_gross_return = nn.Sequential(nn.Linear(hidden_dim, hidden_dim//2), nn.ReLU(), nn.Linear(hidden_dim//2, 1))
        self.head_execution_cost = nn.Sequential(nn.Linear(hidden_dim, hidden_dim//2), nn.ReLU(), nn.Linear(hidden_dim//2, 1))
        self.head_net_edge = nn.Sequential(nn.Linear(hidden_dim, hidden_dim//2), nn.ReLU(), nn.Linear(hidden_dim//2, 1))
        self.head_call_net_edge = nn.Sequential(nn.Linear(hidden_dim, hidden_dim//2), nn.ReLU(), nn.Linear(hidden_dim//2, 1))
        self.head_put_net_edge = nn.Sequential(nn.Linear(hidden_dim, hidden_dim//2), nn.ReLU(), nn.Linear(hidden_dim//2, 1))
        self.symbol_calibrator = PerSymbolCalibrator(caps['stock'], hidden_dim)
        self._init_dual_edge_heads_from_net()
    ...
    def _init_dual_edge_heads_from_net(self):
        """Phase 3: warm-start dual heads from legacy net_edge head (checkpoint-compat)."""
        try:
            self.head_call_net_edge.load_state_dict(self.head_net_edge.state_dict())
            self.head_put_net_edge.load_state_dict(self.head_net_edge.state_dict())
        except Exception:
            pass
    ...
    def forward(self, x_stock, x_option, static_x):
        ...
        calibrated_net_edge, symbol_context = self.symbol_calibrator(static_x['stock_id'], raw_net_edge)
        rank_score = self.head_rank(fused + 0.10 * symbol_context)
        ...
        return {
            ...
            'call_net_edge': call_net_edge,
            'put_net_edge': put_net_edge,
        }
...
class UnifiedLMDBDataset(Dataset):
    ...
    def _sanity_check(self):
        self._init_env()
        check_count = min(1000, len(self.keys))
        zero_ret_count = 0
        missing_net_count = 0
        ...
        for i in range(check_count):
            ...
            lbl = data.get('labels', {})
            if 'label_return_fwd_net' not in lbl:
                missing_net_count += 1
            if 'label_execution_cost' not in lbl:
                missing_cost_count += 1
            r_val = lbl.get('label_return_fwd_net', lbl.get('label_return_fwd', lbl.get('return_fwd', 0.0)))
            c_val = lbl.get('label_execution_cost', 0.0)
            if abs(r_val) < 1e-9: zero_ret_count += 1
            ret_vals.append(float(r_val))
            cost_vals.append(float(c_val))
        if zero_ret_count == check_count:
            logger.warning(f"🚨 [WARNING] 前 {check_count} 条数据的 net return 全为 0！")
        if missing_net_count > 0 or missing_cost_count > 0:
            logger.warning(
                f"Cost-aware labels missing in sample | missing_net={missing_net_count}/{check_count}, "
                f"missing_cost={missing_cost_count}/{check_count}. 将使用旧标签/0成本兜底。"
            )
        if ret_vals:
            logger.info(
                f"Label Stats Sample | net_ret mean={np.mean(ret_vals):.6f}, std={np.std(ret_vals):.6f}, "
                f"cost mean={np.mean(cost_vals):.6f}, cost p95={np.percentile(cost_vals, 95):.6f}"
            )
    ...
    def __getitem__(self, idx):
        ...
        lbl = data.get('labels', {})
        r_val = lbl.get('label_return_fwd_net', lbl.get('label_return_fwd', lbl.get('return_fwd', 0.0)))
        gross_val = lbl.get('label_return_fwd_gross', lbl.get('label_return_fwd', lbl.get('return_fwd', 0.0)))
        cost_val = lbl.get('label_execution_cost', 0.0)
        d_val = lbl.get('label_direction_net', lbl.get('label_direction', lbl.get('direction', 1)))
        if r_val is None or np.isnan(r_val): r_val = 0.0
        if gross_val is None or np.isnan(gross_val): gross_val = r_val
        if cost_val is None or np.isnan(cost_val): cost_val = 0.0
        
        tgt = {
            'direction': int(d_val),
            'return_fwd': float(r_val),
            'return_fwd_gross': float(gross_val),
            'execution_cost': float(cost_val),
        }
...
class StrategicAlphaLoss(nn.Module):
    def __init__(self, config=None):
        ...
        loss_weights = (config or {}).get('loss_weights', {})
        self.w_dir = float(loss_weights.get('direction_net', loss_weights.get('direction', 4.0)))
        self.w_net = float(loss_weights.get('return_fwd_net', 3.0))
        self.w_gross = float(loss_weights.get('return_fwd_gross', 0.5))
        self.w_cost = float(loss_weights.get('execution_cost', 0.5))
        self.w_rank = float(loss_weights.get('rank_net', 2.0))
        self.w_call_put = float(loss_weights.get('call_put_edge', 0.0))
        self.return_beta = float((config or {}).get('parameters', {}).get('training', {}).get('return_huber_beta', 0.001))
        self.rank_temp = float((config or {}).get('parameters', {}).get('training', {}).get('rank_temperature', 0.002))
        self.rank_min_delta = float((config or {}).get('parameters', {}).get('training', {}).get('rank_min_delta', 0.0002))
    ...
    def forward(self, out, target):
        ...
        p = out['net_edge'].squeeze(-1)
        r = net_r
        pred_gross = out['gross_return'].squeeze(-1)
        pred_cost = out['execution_cost'].squeeze(-1)
        pred_raw_edge = out['net_edge_raw'].squeeze(-1)
        executable_target = torch.sign(gross_r) * executable_edge
        l_net = F.smooth_l1_loss(p, r, beta=self.return_beta)
        l_gross = F.smooth_l1_loss(pred_gross, gross_r, beta=self.return_beta)
        l_cost = F.smooth_l1_loss(pred_cost, exec_cost, beta=self.return_beta)
        l_raw_consistency = F.smooth_l1_loss(pred_raw_edge, executable_target, beta=self.return_beta)
        
        if len(p) > 1:
            idx = torch.randperm(len(p))
            diff = r - r[idx]
            mask = torch.abs(diff) > self.rank_min_delta
            if mask.any():
                target_sign = torch.sign(diff[mask])
                pred_diff = p[mask] - p[idx][mask]
                edge_weight = torch.clamp(torch.abs(diff[mask]) / self.rank_min_delta, max=10.0)
                l_rank = (F.softplus(-target_sign * pred_diff / self.rank_temp) * edge_weight).mean()
            else:
                l_rank = torch.tensor(0.0, device=p.device)
        else:
            l_rank = torch.tensor(0.0, device=p.device)

        l_call_put = torch.tensor(0.0, device=p.device)
        if self.w_call_put > 0 and 'call_net_edge' in out and 'put_net_edge' in out:
            call_ret = torch.nan_to_num(target.get('call_return_fwd', torch.zeros_like(net_r)), nan=0.0)
            put_ret = torch.nan_to_num(target.get('put_return_fwd', torch.zeros_like(net_r)), nan=0.0)
            call_pred = out['call_net_edge'].squeeze(-1)
            put_pred = out['put_net_edge'].squeeze(-1)
            l_call_put = (
                F.smooth_l1_loss(call_pred, torch.clamp(call_ret, min=0.0), beta=self.return_beta)
                + F.smooth_l1_loss(put_pred, torch.clamp(put_ret, min=0.0), beta=self.return_beta)
            ) * 0.5
        ...
        loss = (
            ...
            + self.w_cost * l_cost
            + 0.5 * l_raw_consistency
            + self.w_call_put * l_call_put
        )
        return loss, l_rank.item()

def validate(model, loader, device):
    model.eval()
    preds, rets, gross_rets, costs, tss, sids = [], [], [], [], [], []
    with torch.no_grad():
        for b in tqdm(loader, desc="Val", leave=False):
            ...
            preds.extend(o['net_edge'].cpu().numpy().flatten())
            rets.extend(t['return_fwd'].numpy().flatten())
            gross_rets.extend(t.get('return_fwd_gross', t['return_fwd']).numpy().flatten())
            costs.extend(t.get('execution_cost', torch.zeros_like(t['return_fwd'])).numpy().flatten())
            tss.extend(ts)
            sids.extend(s['stock_id'].detach().cpu().numpy().flatten())
            
    df = pd.DataFrame({'p': preds, 'r': rets, 'g': gross_rets, 'c': costs, 't': tss, 'sid': sids})
    logger.info(
        f"[Val] NetRet mean={df['r'].mean():.6f}, std={df['r'].std():.6f}, "
        f"Gross mean={df['g'].mean():.6f}, Cost mean={df['c'].mean():.6f}, Pred std={df['p'].std():.6f}"
    )

    if df['r'].std() < 1e-9:
        logger.warning("⚠️ 验证集 net return 方差接近 0，请优先检查 LMDB 标签。")
    if df['p'].std() < 1e-9:
        logger.warning("⚠️ 模型 net_edge 预测方差接近 0，可能发生输出坍塌。")

    def safe_corr(x, method='spearman'):
        if len(x) <= 20 or x['p'].std() < 1e-9 or x['r'].std() < 1e-9:
            return np.nan
        return x['p'].corr(x['r'], method=method)

    global_ic = safe_corr(df, method='spearman')
    per_symbol_ic = df.groupby('sid')[['p', 'r']].apply(lambda x: safe_corr(x, method='spearman'))
    ic_mean = per_symbol_ic.dropna().mean()
    if np.isnan(ic_mean):
        ic_mean = 0.0 if np.isnan(global_ic) else global_ic
    ...
    top_hit = 0.0
    if len(df) > 50:
        n = max(10, int(len(df) * 0.05))
        top = df.nlargest(n, 'p')
        top_mean = top['r'].mean()
        top_hit = (top['r'] > 0).mean()

    logger.info(
        f"[Val] GlobalIC={global_ic if not np.isnan(global_ic) else 0.0:.4f}, "
        f"PerSymbolIC={ic_mean:.4f}, Spread10={spread:.6f}, Top5Mean={top_mean:.6f}, Top5Hit={top_hit:.2%}"
    )
    return {'ic': ic_mean, 'global_ic': 0.0 if np.isnan(global_ic) else global_ic, 'spread': spread, 'top_mean': top_mean, 'top_hit': top_hit}


def load_meta_info():
    if not PG_DB_URL:
        logger.warning("PG_DB_URL 未配置，使用默认 stock/sector embedding 容量。")
...
def load_checkpoint(model, optimizer, scheduler, checkpoint_dir):
    ...
    try:
        ...
        if missing or unexpected:
            logger.warning(f"Checkpoint partial load | missing={len(missing)} unexpected={len(unexpected)}")
        try:
            optimizer.load_state_dict(checkpoint['optimizer'])
            if scheduler and 'scheduler' in checkpoint:
                scheduler.load_state_dict(checkpoint['scheduler'])
        except Exception as opt_e:
            logger.warning(f"Optimizer/scheduler state skipped after architecture change: {opt_e}")
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train cost-aware TFT net-edge model.")
    parser.add_argument("--mode", choices=["train", "fine_tune"], default="train")
    parser.add_argument("--config", default=None)
    parser.add_argument("--data-root", default="/mnt/s990/data/h5_unified_overlap_id")
    parser.add_argument("--train-lmdb", default="train_quote_alpha.lmdb")
    parser.add_argument("--val-lmdbs", default="val_quote_alpha.lmdb", help="逗号分隔的验证 LMDB 文件名")
    parser.add_argument("--checkpoint-dir", default="./checkpoints_net_edge")
    ...
    parser.add_argument("--resume", action="store_true", help="显式允许从 latest checkpoint 续训")
...
def fine_tune(args):
    ...
    val_filenames = [x.strip() for x in args.val_lmdbs.split(',') if x.strip()]
    ...
    # 🚨 [修复] 保持与 main() 数据集名称严格一致
    train_ds = UnifiedLMDBDataset(str(h5_root / args.train_lmdb), config)
     
    train_dl = DataLoader(train_ds, batch_size=args.batch_size, shuffle=True, collate_fn=collate_fn, num_workers=args.num_workers, pin_memory=(device.type == 'cuda'))
    val_dl = DataLoader(val_ds, batch_size=args.batch_size, shuffle=False, collate_fn=collate_fn, num_workers=max(1, args.num_workers // 2), pin_memory=(device.type == 'cuda'))
    ...
    crit = StrategicAlphaLoss(config).to(device)
...
def main(args):
    ...
    train_path = h5 / args.train_lmdb
    if not train_path.exists():
        raise FileNotFoundError(f"训练集不存在: {train_path}")
    train_dl = DataLoader(
        UnifiedLMDBDataset(str(train_path), config),
    ...
    scheduler = OneCycleLR(optim, max_lr=args.max_lr, total_steps=max(1, args.epochs*len(train_dl)), pct_start=0.1, div_factor=25)
    if args.resume:
        start_ep, best_ic = load_checkpoint(model, optim, scheduler, ckpt_dir)
    else:
        start_ep, best_ic = 0, -1.0
        logger.info("Starting fresh. Use --resume only when continuing the same architecture/run.")
    ...
    for ep in range(start_ep, args.epochs):
        ...
        ic = metrics['ic']
        logger.info(
            f"Ep {ep}: Loss={np.mean(logs):.4f}, IC={ic:.4f}, "
            f"GlobalIC={metrics['global_ic']:.4f}, Spr={metrics['spread']:.6f}, Top5={metrics['top_mean']:.6f}"
        )
