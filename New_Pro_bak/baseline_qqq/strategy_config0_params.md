...
| `REGIME_ADAPTIVE_STOCK_STOP_ENABLED` | `True` | 是否根据市场 regime 自动切换正股硬止损档位；当前启用，按 `calm / mixed / volatile` 三档切换。 |
| `REGIME_REVERSAL_THRESHOLD` | `6` | 指定窗口内 VIXY 反转次数超过该值时标记 VIXY 波动候选。 |
| `REGIME_WINDOW_MINS` | `30` | 反转统计窗口。 |
| `REGIME_REVERSAL_PERCENT` | `0.0015` | 判定一次反转的涨跌幅阈值。 |
...
| `REGIME_MIXED_SCORE_THRESHOLD` | `0.60` | regime 分数超过该值进入 `mixed` 候选档。 |
| `REGIME_VOLATILE_SCORE_THRESHOLD` | `1.00` | regime 分数超过该值进入 `volatile` 候选档。 |
| `REGIME_BAND_ENTER_CONFIRM_BARS` | `2` | 进入更紧档位前，连续确认 bar 数。 |
| `REGIME_BAND_EXIT_CONFIRM_BARS` | `4` | 从更紧档位退出前，连续确认 bar 数。 |

V0 现在将 regime 分成 `calm / mixed / volatile` 三档，并带有简单滞后确认。Entry Guard 仍只负责拦截最差的 `volatile` 入场环境；正股 hard stop 则会按三档自动切换，避免过去“要么完全不收紧、要么一刀切过紧”的问题。
...
| `STOCK_HARD_STOP_TIGHT_MIXED` | `0.0022` | `mixed` 档普通置信度正股止损。 |
| `STOCK_HARD_STOP_LOOSE_MIXED` | `0.004` | `mixed` 档高置信度正股止损。 |
| `STOCK_HARD_STOP_TIGHT_VOLATILE` | `0.0015` | `volatile` 档普通置信度正股止损。 |
| `STOCK_HARD_STOP_LOOSE_VOLATILE` | `0.003` | `volatile` 档高置信度正股止损。 |
