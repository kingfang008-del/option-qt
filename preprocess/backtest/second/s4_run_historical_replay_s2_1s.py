...
# 分钟 alpha 左对齐标签：+60s 后再与 1s 行情 backward merge，避免同一分钟内 lookahead。
# 与实盘 FCS「alpha_label_ts = current_minute_ts - 60」语义一致，见 baseline/replay_live_parity_utils.py。
ALPHA_AVAILABLE_DELAY_SECONDS = 60.0


def resolve_db_path(args):
    """Resolve SQLite directory: HISTORY_SQLITE_1S_DIR, then legacy path, then repo backtest dir."""
    current_dir = Path(__file__).resolve().parent
    env = (os.environ.get("HISTORY_SQLITE_1S_DIR") or "").strip()
    if env:
        hist_dir = Path(env).expanduser()
    else:
        legacy = Path("/home/kingfang007/quant_project/data/history_sqlite_1s")
        hist_dir = legacy if legacy.is_dir() else (current_dir.parent / "history_sqlite_1s")
    if args.date:
