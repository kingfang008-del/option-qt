...
def main() -> None:
    ...
    p.add_argument("--compare-offline", action="store_true", help="also run offline single & diff trades")
    args = p.parse_args()
    ...
    tag = args.tag or f"oms_dry_{args.ingest}_{args.start_date}_{end}"
    ...
    summary = runner.process_signals(scanner.signals)
    ...
    for t in runner.trades[:20]:
        print(
            ...
            f"ret={t.ret:+.1%} {t.reason} entry={t.entry:.2f} exit={t.exit:.2f}"
        )
