...
class IBKRConnectorFinal:
            ...
            for r in rows:
                ...
                try:
                    ...
                    from anchor_config import (
                        anchor_profile_active,
                        min_front_dte_for_restore,
                        use_next_buckets,
                    )
                    if anchor_profile_active() and 'NEXT' in tag and not use_next_buckets():
                        logger.info(f"⏭️ 跳过 legacy NEXT 锁 (0DTE profile): {sym} {tag}")
                        continue

                    min_front_dte = min_front_dte_for_restore()
                    if is_front and dte < min_front_dte:
                        logger.warning(
                            f"⚠️ 拒绝恢复 front 合约 (DTE={dte} < {min_front_dte}): {sym} {tag}"
                        )
                        continue
                except Exception as e:
    ...
    async def _process_single_symbol(self, sym: str):
        ...
        try:
            ...
            if not is_index:
                from anchor_config import get_required_lock_tags
                required_tags = get_required_lock_tags()
                missing_tags = [
                    tag for tag in required_tags
                    if tag not in self.locked_contracts.get(original_key, {})
                ]
                if missing_tags:
                    # 获取现价用于搜索
    ...
    async def _find_contracts(self, sym, spot):
        ...
        # =========================================================
        # 像素级对齐离线 step1 / anchor_qqq_0dte.json（默认 0DTE 4-bucket）
        # =========================================================
        from anchor_config import (
            anchor_profile_active,
            get_anchor_config,
            get_live_bucket_specs,
            pick_front_expiration,
        )

        if anchor_profile_active():
            ANCHOR_CONFIG = get_anchor_config()
            use_next_buckets_flag = bool(ANCHOR_CONFIG.get("use_next_buckets", False))
            min_exp_dte = int(ANCHOR_CONFIG.get("front_min_dte", 0))
            atm_center = float(ANCHOR_CONFIG.get("atm_center", 0.50))
            otm_center = float(ANCHOR_CONFIG.get("otm_center", 0.25))
            delta_tolerance = float(ANCHOR_CONFIG.get("delta_tolerance", 0.15))
        else:
            ANCHOR_CONFIG = {
                "FRONT_TARGET_DTE": 9,
                "FRONT_MIN_DTE": 5,
                ...
                "NEXT_MIN_GAP": 20,
                "NEXT_MAX_GAP": 50,
                "use_next_buckets": True,
                "atm_center": 0.50,
                "otm_center": 0.25,
            }
            use_next_buckets_flag = True
            min_exp_dte = 2
            atm_center = 0.50
            otm_center = 0.25
        ...
        # --- A. 选 Front expiry ---
        if anchor_profile_active():
            selected_front = pick_front_expiration(exp_dtes)
            if selected_front is None:
                return {}
        else:
            front_candidates = [
                x for x in exp_dtes
                if ANCHOR_CONFIG["FRONT_MIN_DTE"] <= x[1] <= ANCHOR_CONFIG["FRONT_MAX_DTE"]
            ]
            if front_candidates:
                selected_front = min(
                    front_candidates,
                    key=lambda x: abs(x[1] - ANCHOR_CONFIG["FRONT_TARGET_DTE"]),
                )
            else:
        ...
        logger.info(f"📅 [Anchor] {sym} front expiry={selected_front[0]} DTE={actual_front_dte}")

        # --- B. 选 Next（仅 legacy 6-bucket profile）---
        target_exps_next = []
        if use_next_buckets_flag:
            ...
            if next_candidates:
                ...
                if fallbacks:
                    ...
                    logger.warning(
                        f"❌ API 无法找到 {sym} 大于 {actual_front_dte} 天的期权，放弃 NEXT 合约订阅。"
                    )
        ...
        # 3. 扫描各 Bucket（0DTE profile 仅 front 4 档）
        live_specs = get_live_bucket_specs() if anchor_profile_active() else BUCKET_SPECS

        for tag, spec in sorted(live_specs.items(), key=lambda x: x[1]['bucket_idx']):
            is_front = 'NEXT' not in tag
            if not is_front and not target_exps_next:
                continue
            target_exps = target_exps_front if is_front else target_exps_next
            
            right = 'C' if 'CALL' in tag else 'P'
            target_delta = atm_center if 'ATM' in tag else otm_center
            ...
            df_good_delta = df[df['delta_err'] < delta_tolerance].copy()
    ...
    async def _find_contracts_from_pg(self, sym):
        ...
        0DTE profile 仅恢复 front 4 bucket；legacy 恢复 6 bucket。
        """
        import json
        import re
        from anchor_config import get_live_bucket_specs, use_next_buckets as _use_next_buckets
        ...
        try:
            ...
            c.execute(
                "SELECT buckets_json, ts FROM option_snapshots_1m WHERE symbol=%s AND ts >= %s ORDER BY ts DESC LIMIT 1",
                (sym, today_start),
            )
            row = c.fetchone()
            conn.close()
            ...
            opt_data, snap_ts = row
            if isinstance(opt_data, str):
                opt_data = json.loads(opt_data)

            contract_ids = opt_data.get('contracts', [])
            if not isinstance(contract_ids, list) or not any(contract_ids):
                return {}

            live_specs = get_live_bucket_specs()
            max_idx = max(spec['bucket_idx'] for spec in live_specs.values())
            if len(contract_ids) <= max_idx:
                return {}
            ...
            for tag, spec in live_specs.items():
                idx = spec['bucket_idx']
                if idx >= len(contract_ids):
                    continue
                con_id = contract_ids[idx]
                if not con_id:
                    continue

                clean_id = str(con_id).replace('O:', '')
                match = re.match(r'^([A-Z]{1,6})(\d{6}[CP]\d{8})$', clean_id)
                if match:
                    sym_root, rest = match.groups()
                    clean_id = sym_root.ljust(6, ' ') + rest

                c_opt = Option(symbol=sym, exchange='SMART', currency='USD')
                c_opt.localSymbol = clean_id
                found_contracts[tag] = c_opt
            ...
            if _use_next_buckets():
                if 'CALL_ATM' in found_contracts and 'NEXT_CALL_ATM' in found_contracts:
                    if found_contracts['CALL_ATM'].localSymbol == found_contracts['NEXT_CALL_ATM'].localSymbol:
                        logger.warning(
                            "⚠️ 数据库 EOD 缓存近远月发生重叠，拒绝使用，将转入 API 在线搜索。"
                        )
                        return {}
    ...
    async def maintenance_loop(self, interval=60):
        ...
        while True:
            ...
            for sym, contract in list(self.active_stocks.items()):
                ...
                from anchor_config import get_required_lock_tags, use_next_buckets as _use_next_buckets
                ...
                if _use_next_buckets():
                    if 'CALL_ATM' in locks and 'NEXT_CALL_ATM' in locks:
                        if locks['CALL_ATM'].lastTradeDateOrContractMonth == locks['NEXT_CALL_ATM'].lastTradeDateOrContractMonth:
                            logger.warning(
                                f"🚨 检测到 {sym} 缓存锁中毒 (近月远月挤在同一天)，撕毁缓存，强制重新搜索！"
                            )
                            self.locked_contracts[sym] = {}

                missing = [
                    tag for tag in get_required_lock_tags()
