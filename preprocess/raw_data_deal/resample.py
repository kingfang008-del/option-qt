...
# 阈值设置：针对日内滞后数据的判定阈值 (18%)
ANTI_LAG_THRESHOLD = 0.18
...
def resample_data(df, freq, time_range, symbol=None, file_name=None):
    """
    重采样数据，并内嵌 Anti-Lag 修复。已修复缺失列和维度不一致问题。
    """
    # 1. 检查必备列
    ...
    full_time_index = pd.date_range(
        start=pd.Timestamp(f"{date} {time_range[0]}", tz=resampled_df.index.tz),
    ...
    # 强制转换数据格式，防止后续 concat 维度匹配错误与 pyarrow 写库报错
...
def process_symbol(symbol):
    minute_dir = Path.home() / "train_data/spnq_train" / symbol
    second_dir = Path.home() / "train_data/spnq_stock_test_extended_hours" / symbol
    output_base = Path.home() / "train_data/spnq_train_resampled"

    if not minute_dir.exists() and not second_dir.exists():
        log_error(f"Skipped {symbol}: No data directories found.")
        return f"Skipped {symbol}"
    
    all_files = []
    if minute_dir.exists():
        all_files.extend([(p, 'minute') for p in minute_dir.glob("*.parquet")])
    if second_dir.exists():
        all_files.extend([(p, 'second') for p in second_dir.glob("*.parquet")])

    for file_path, data_type in all_files:
        file_name = file_path.name
        match = re.match(r"(\d{4})-(\d{2})\.parquet", file_name)
        if not match:
            continue
        ...
        try:
            ...
            if 'timestamp' in df.columns:
                ...
                log_error(f"No timestamp column in {symbol} - {file_name}")
                continue
        except Exception as e:
            log_error(f"Read PARQUET error for {symbol} - {file_name}: {e}\n{traceback.format_exc()}")
            continue

        sessions_to_process = ["regular"] if data_type == 'minute' else ["pre_market", "after_hours"]
                        ...
                        try:
                            ...
                            for d, daily_df in daily_groups:
                                ...
                                if not resampled_day_df.empty and len(resampled_day_df.columns) == 8:
                                    ...
                                    log_error(f"Dimension Mismatch {symbol} {file_name} {d}: Columns {list(resampled_day_df.columns)}")
                            ...
                            if not resampled_df.empty:
                                output_dir = output_base / symbol / session / time_range / freq
                                output_dir.mkdir(parents=True, exist_ok=True)
                                output_path = output_dir / f"{year}-{month:02d}.parquet"
                                
                                resampled_df.to_parquet(output_path, index=False)
                        except Exception as e:
                            log_error(f"Concat/Save error for {symbol} {file_name} {session} {freq}: {e}\n{traceback.format_exc()}")
...
def main():
    ...
    try:
     ...
     # 动态生成占位符并执行查询
        ...
        print(f"无法从数据库加载股票列表 ({e})，将使用默认列表。")
    ...
    print(f"共找到 {len(symbols)} 个股票代码。使用 {max_workers} 个进程开始并行处理...")
