...
class S5ParquetDriver:
    ...
    def run(self):
        ...
        # [🔥 终极修正] 使用字符串格式化强制“秒级”截面对齐 (1s 精度)
        df['dt'] = pd.to_datetime(df['ts'], unit='s', utc=True).dt.tz_convert('America/New_York')
        df['ts_group'] = df['dt'].dt.strftime('%Y%m%d %H:%M:%S')
        
        unique_groups = df['ts_group'].nunique()
        logger.info(f"🔍 [DEBUG] Unique groups using YYYYMMDD HH:MM:SS string: {unique_groups}")
        
        # 还要保留原始的 float TS 供 SE 使用
        grouped = df.groupby('ts_group')
        
        count = 0
        last_minute = None
        for group_name, group in tqdm(grouped, desc="Replaying 1s Ticks"):
            # 取该秒内第一个点的原始浮点时间戳作为逻辑时间戳
            ts_val = float(group['ts'].iloc[0])
            
            # 🚀 [新增] 动态计算“新分钟”标志 (用于同步 Orchestrator 的 EWMA 降频)
            current_minute = group_name[:14] # 取到 YYYYMMDD HH:MM
            is_new_minute = (current_minute != last_minute)
