...
class MockIBKRHistorical:
    ...
    @staticmethod
    def _coalesce_opt_bid_ask_like_execution_engine(bid, ask, last_px):
        """与 ExecutionEngineV8 开仓定价一致：任一侧 bid/ask ≤0 则双边改用 last（feed 价）。"""
        try:
            b = float(bid)
            a = float(ask)
            t = float(last_px)
    ...
    def _get_price_at_time(self, symbol, ts, opt_dir, action, price_type='mid'):
        ...
        if price_type == 'mid':
            ...
            if opt_dir == 'CALL':
                cb, ca = self._coalesce_opt_bid_ask_like_execution_engine(
                    data.get('cb', 0.0), data.get('ca', 0.0), data.get('c', 0.0)
                )
                if cb > 0 and ca > 0:
                    px = self._spread_interpolate_fill(cb, ca, action, frac)
                    if px is not None:
                        return float(px)
                return data['c']
            else:
                pb, pa = self._coalesce_opt_bid_ask_like_execution_engine(
                    data.get('pb', 0.0), data.get('pa', 0.0), data.get('p', 0.0)
                )
                if pb > 0 and pa > 0:
                    px = self._spread_interpolate_fill(pb, pa, action, frac)
                    if px is not None:
                        return float(px)
                return data['p']
    ...
    def _print_summary_report(self, df, open_pos):
        ...
        max_dd = drawdown.min()
        mtm_max_dd = self._calc_liquidation_mtm_drawdown()
        ...
        if mtm_max_dd is not None:
            print(f"盘中MTM最大回撤(按Bid清算): {mtm_max_dd:.2%}")
    ...
    def _calc_liquidation_mtm_drawdown(self):
        """
        Closed-trade drawdown ignores intratrade option marks. This liquidation
        view marks open long options at bid each recorded second, matching the
        risk seen by realtime dry-run dashboards more closely.
        """
        if not self.orders or not self.market_history:
            return None

        cash = float(self.initial_capital)
        open_lots = {}
        orders = sorted(self.orders, key=lambda x: float(x.get('ts', 0.0) or 0.0))
        order_idx = 0
        curve = []

        def mark_lot(sym, lot, snapshot):
            data = snapshot.get(sym) or {}
            opt_dir = str(lot.get('opt_dir', 'CALL')).upper()
            if opt_dir == 'CALL':
                bid = float(data.get('cb', 0.0) or 0.0)
                last = float(data.get('c', 0.0) or 0.0)
            else:
                bid = float(data.get('pb', 0.0) or 0.0)
                last = float(data.get('p', 0.0) or 0.0)
            px = bid if bid > 0.0 else last
            return max(px, 0.0) * int(lot.get('qty', 0) or 0) * 100.0

        for ts, snapshot in self.market_history:
            ts = float(ts)
            while order_idx < len(orders) and float(orders[order_idx].get('ts', 0.0) or 0.0) <= ts:
                od = orders[order_idx]
                order_idx += 1
                ...
                if qty <= 0 or price <= 0.0:
                    continue
                if str(od.get('action', '')).upper() == 'BUY':
                    cash -= price * qty * 100.0 + qty * COMMISSION_PER_CONTRACT
                    open_lots.setdefault(sym, []).append({
                        'qty': qty,
                        'opt_dir': od.get('opt_dir', 'CALL'),
                    })
                else:
                    cash += price * qty * 100.0 - qty * COMMISSION_PER_CONTRACT
                    remaining = qty
                    ...
                    while remaining > 0 and lots:
                        take = min(int(lots[0].get('qty', 0) or 0), remaining)
                        lots[0]['qty'] = int(lots[0].get('qty', 0) or 0) - take
                        remaining -= take
                        if int(lots[0].get('qty', 0) or 0) <= 0:
                            lots.pop(0)
                    if not lots and sym in open_lots:
                        open_lots.pop(sym, None)

            mtm = cash
            for sym, lots in open_lots.items():
                for lot in lots:
                    mtm += mark_lot(sym, lot, snapshot)
            curve.append(mtm)

        if not curve:
            return None
        eq = pd.Series(curve, dtype='float64')
