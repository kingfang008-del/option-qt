...
def _resolve_price_array(df: pd.DataFrame) -> np.ndarray:
    """Best-effort raw price array for optional data-quality checks."""
...
def _ensure_lmdb_label_columns(df: pd.DataFrame, config: dict) -> tuple[pd.DataFrame, list[str]]:
    """
    LMDB training labels must never be silently zero-filled.  If labeled feature
    files are not available, synthesize a gross forward-return label from price
    or close_log_return so the training job fails less mysteriously.
    """
    ...
    if 'label_return_fwd' not in df.columns:
        ...
        if price_col:
            ...
            valid = (np.arange(n) + k < n) & (px > 1e-12)
            gross[valid] = (px[np.arange(n)[valid] + k] / px[np.arange(n)[valid]] - 1.0).astype(np.float32)
            ...
            if valid_count:
                ...
                future_log_ret = cs[idx + k + 1] - cs[idx + 1]
                gross[:valid_count] = np.expm1(future_log_ret).astype(np.float32)
        else:
    ...
    for c in label_cols:
        if c not in df.columns:
            raise ValueError(f"Missing required label column after label synthesis: {c}")

    active_label_cols = label_cols + [c for c in cost_label_cols if c in df.columns]
        ...
        for (session_name, time_range, res), file_list in files_by_group.items():
            ...
            for f_1m in file_list:
                ...
                try:
                    ...
                    # [优化] 预先提取价格数组，专门用于死水检查。
                    # ret_cost 特征文件可能只保留特征列而无原始 close，因此这里做宽松 fallback。
