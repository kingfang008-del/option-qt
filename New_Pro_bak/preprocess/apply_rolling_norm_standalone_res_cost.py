...
# 配置文件（优先 New_Pro slow_feature.json）
_SCRIPT_DIR = Path(__file__).resolve().parent
CONFIG_PATH = _SCRIPT_DIR.parent / "CONFIG" / "slow_feature.json"
...
# 写回熔断：原始特征有波动，但归一化后几乎全 0 时，不覆盖源文件。
ZERO_RATIO_ABORT_THRESHOLD = 0.98
RAW_STD_SIGNAL_THRESHOLD = 1e-8
# ===========================================
...
def process_single_directory(args):
    ...
    try:
        ...
        for file_path in files:
            ...
            # 填充 NaN。注意：NaN 通常表示 rolling min_periods 不足或 std 不可用。
            current_z = current_z.fillna(0.0)

            raw_std = df[cols_to_process].std(axis=0, skipna=True)
            signal_cols = raw_std[raw_std > RAW_STD_SIGNAL_THRESHOLD].index.tolist()
            if signal_cols:
                zero_ratio = (np.abs(current_z[signal_cols].values) <= 1e-12).mean()
                if zero_ratio >= ZERO_RATIO_ABORT_THRESHOLD:
                    sample_cols = ", ".join(signal_cols[:8])
                    return (
                        f"ERROR in {file_path}: rolling norm produced {zero_ratio:.2%} zeros "
                        f"on non-constant columns [{sample_cols}]. Skip write to avoid data loss. "
                        f"Check MIN_PERIODS={MIN_PERIODS}, input variance, and whether data was already normalized."
                    )
...
def main():
    ...
    # --- 循环处理每个 Stage ---
    for stage in STAGES:
        stage_dir_name = f"ret_cost_quote_features_{stage}"
