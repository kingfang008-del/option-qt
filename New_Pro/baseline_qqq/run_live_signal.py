...
import baseline_paths  # noqa: F401
...
async def main():
    ...
    config_paths = {
        'fast': str(CONFIG_DIR / "fast_feature_qqq.json"),
        'slow': str(CONFIG_DIR / "slow_feature.json"),
    }
    for key, path in list(config_paths.items()):
        if not Path(path).exists():
            fallback = HOME / "notebook/train" / ("fast_feature.json" if key == 'fast' else "slow_feature.json")
            config_paths[key] = str(fallback)
    model_paths = {
        'slow': str(PROJECT_ROOT / "checkpoints_qqq_net_edge/advanced_alpha_best.pth"),
    }
    if not Path(model_paths['slow']).exists():
        model_paths['slow'] = str(PROJECT_ROOT / "checkpoints_advanced_alpha/advanced_alpha_best.pth")
    ...
    try:
        ...
        run_startup_cleanup(role="se", dry_run=_dry)
