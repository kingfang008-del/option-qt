...
def _resolve_price_array(df: pd.DataFrame) -> np.ndarray:
    """Best-effort raw price array for label synthesis and optional data checks."""
    for col in ("close", "price", "vwap", "open"):
        if col in df.columns:
            return pd.to_numeric(df[col], errors="coerce").ffill().fillna(0.0).values.astype(np.float32)
    return np.zeros(len(df), dtype=np.float32)


def _label_horizon(config: dict) -> int:
    return int(
        config.get('parameters', {})
        .get('slow_channel', {})
        .get('label_horizon_k', 5)
    )


def _ensure_lmdb_label_columns(df: pd.DataFrame, config: dict) -> tuple[pd.DataFrame, list[str]]:
    """
    Make S0 explicit about cost-aware labels.

    Missing labels should not be silently zero-filled. If old feature parquet
    files are used, synthesize a conservative gross/net return from price or
    close_log_return so the model has a real target.
    """
    base_label_cols = ['label_direction', 'label_return_fwd', 'label_volatility', 'label_event']
    cost_label_cols = [
        'label_direction_net',
    ...
    if 'label_return_fwd' not in df.columns:
        k = max(1, _label_horizon(config))
        n = len(df)
        gross = np.zeros(n, dtype=np.float32)
        price_col = next((c for c in ('close', 'price', 'vwap', 'open') if c in df.columns), None)
        if price_col:
            px = pd.to_numeric(df[price_col], errors='coerce').ffill().to_numpy(dtype=np.float64)
            idx = np.arange(n)
            valid = (idx + k < n) & (px > 1e-12)
            gross[valid] = (px[idx[valid] + k] / px[idx[valid]] - 1.0).astype(np.float32)
        elif 'close_log_return' in df.columns:
            lr = pd.to_numeric(df['close_log_return'], errors='coerce').fillna(0.0).to_numpy(dtype=np.float64)
            cs = np.concatenate([[0.0], np.cumsum(lr)])
            valid_count = max(0, n - k)
            if valid_count:
                idx = np.arange(valid_count)
                gross[:valid_count] = np.expm1(cs[idx + k + 1] - cs[idx + 1]).astype(np.float32)
        else:
            raise ValueError("Missing label_return_fwd and no price/close_log_return column to synthesize labels")
        ...
        threshold = (
            ...
            .get('resolutions', {})
            .get('1min', {})
            .get('entry_threshold', 0.0015)
        )
        df['label_return_fwd'] = gross
        df['label_return_fwd_gross'] = gross
        df['label_execution_cost'] = 0.0
        df['label_return_fwd_net'] = gross
        df['label_direction'] = np.where(gross > threshold, 2, np.where(gross < -threshold, 0, 1)).astype(np.int8)
        df['label_direction_net'] = df['label_direction']
        if 'close_log_return' in df.columns:
            lr_s = pd.to_numeric(df['close_log_return'], errors='coerce').fillna(0.0)
            df['label_volatility'] = lr_s.rolling(window=k).std().shift(-k).fillna(0.0).astype(np.float32)
        else:
            df['label_volatility'] = 0.0
        df['label_event'] = 0

    for c in base_label_cols:
        if c not in df.columns:
            raise ValueError(f"Missing required label column after synthesis: {c}")

    active_label_cols = base_label_cols + [c for c in cost_label_cols if c in df.columns]
    return df, active_label_cols
        ...
        for (session_name, time_range, res), file_list in files_by_group.items():
            ...
            for f_1m in file_list:
                ...
                try:
                    ...
                    df_1m, active_label_cols = _ensure_lmdb_label_columns(df_1m, config)
                    arr_1m = to_np(df_1m, feats_1m_cols)
                    arr_5m = to_np(df_5m, feats_5m_cols)
                    arr_lbl = to_np(df_1m, active_label_cols)
                    ...
                    raw_close_arr = _resolve_price_array(df_1m)
                    ...
                    for i in range(start_idx, len(df_1m), WINDOW_STEP):
                        ...
                        sample = {
                            ...
                            'labels': dict(zip(active_label_cols, arr_lbl[i])),
