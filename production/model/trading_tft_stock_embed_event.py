...
class AdvancedAlphaNet(nn.Module):
    def __init__(self, config, caps, hidden_dim=64, dropout=0.3):
        ...
        # [NEW] Event Detection Head (Binary: 0=Normal, 1=Event)
        self.head_event = nn.Sequential(nn.Linear(hidden_dim, hidden_dim//2), nn.ReLU(), nn.Linear(hidden_dim//2, 2))
    ...
    def forward(self, x_stock, x_option, static_x):
        ...
        return {
            ...
            'rank_score': self.head_rank(fused),
            'logits_event': self.head_event(fused)  # [NEW] 返回事件预测概率
        }
...
class UnifiedLMDBDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        e_val = lbl.get('label_event', lbl.get('event', 0)) # [NEW] 提取事件标签
...
def collate_fn(batch):
    ...
    t = {
        ...
        dtype=torch.long if k in ['direction', 'event'] else torch.float32) 
        for k in t_k
    }
...
def validate(model, loader, device):
    ...
    with torch.no_grad():
        for b in tqdm(loader, desc="Val", leave=False):
            ...
            # [NEW] 收集事件预测与标签
            ev_logits.append(o['logits_event'].cpu())
            ev_targets.append(t['event'].cpu())
            
    df = pd.DataFrame({'p': preds, 'r': rets, 't': tss})
    
    # --- 计算事件指标 (Pure Numpy 实现，避免依赖 sklearn) ---
    all_ev_logits = torch.cat(ev_logits, dim=0)
    all_ev_targets = torch.cat(ev_targets, dim=0).numpy().astype(float)
    
    ev_probs = torch.softmax(all_ev_logits, dim=-1)[:, 1].numpy()
    ev_preds = (ev_probs > 0.5).astype(float)
    
    # Precision / Recall
    tp = np.sum((ev_preds == 1) & (all_ev_targets == 1))
    fp = np.sum((ev_preds == 1) & (all_ev_targets == 0))
    fn = np.sum((ev_preds == 0) & (all_ev_targets == 1))
    
    ev_precision = tp / (tp + fp) if (tp + fp) > 0 else 0.0
    ev_recall = tp / (tp + fn) if (tp + fn) > 0 else 0.0
    
    # Simple AUC (using trapezoidal rule on 100 thresholds)
    try:
        thresholds = np.linspace(0, 1, 100)
        tpr_list, fpr_list = [0.0], [0.0]
        for th in thresholds:
            p = (ev_probs >= th).astype(float)
            tp_th = np.sum((p == 1) & (all_ev_targets == 1))
            fp_th = np.sum((p == 1) & (all_ev_targets == 0))
            fn_th = np.sum((p == 0) & (all_ev_targets == 1))
            tn_th = np.sum((p == 0) & (all_ev_targets == 0))
            tpr_list.append(tp_th / (tp_th + fn_th) if (tp_th + fn_th) > 0 else 0.0)
            fpr_list.append(fp_th / (fp_th + tn_th) if (fp_th + tn_th) > 0 else 0.0)
        ...
        ev_auc = np.trapezoid(tpr_sorted, fpr_sorted)
    except: ev_auc = 0.5
    
    logger.info(f"[Val] Event Recall: {ev_recall:.4f} | Precision: {ev_precision:.4f} | AUC: {ev_auc:.4f} | TP={int(tp)}, FP={int(fp)}, FN={int(fn)}")
    ...
    return {'ic': ic_mean, 'spread': spread, 'event_recall': ev_recall, 'event_precision': ev_precision, 'event_auc': ev_auc}
...
def fine_tune():
    ...
    ckpt_dir = Path("./checkpoints_event_alpha")
    ...
    for ep in range(FT_EPOCHS):
        ...
        for b in pbar:
            ...
            pbar.set_postfix({'L': f"{loss.item():.4f}", 'rk': f"{lr_val:.4f}", 'ev': f"{lev_val:.4f}"})
            
        metrics = validate(model, val_dl, device)
        ic = metrics['ic']
        recall = metrics['event_recall']
        # [NEW] 复合评分：IC 占主导，Recall 占加分
        comb_score = ic + recall * 0.1
        
        logger.info(f"FT-Ep {ep}: Loss={np.mean(logs):.4f}, IC={ic:.4f}, Recall={recall:.4f}, Score={comb_score:.4f}")
...
def main():
    ...
    for ep in range(start_ep, 20):
        ...
        ic = metrics['ic']
        recall = metrics['event_recall']
        # [NEW] 复合评分：Score = IC + 0.1 * Recall
        comb_score = ic + recall * 0.1
        
        logger.info(f"Ep {ep}: Loss={np.mean(logs):.4f}, IC={ic:.4f}, Recall={recall:.4f}, Score={comb_score:.4f}")
