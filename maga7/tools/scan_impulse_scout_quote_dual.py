#!/usr/bin/env python3
"""Quote FillSpec dual for impulse scout DN champions (trades PASS first).

Independent scout — not Mag7 Rule-A. Sessions match trades scan.
"""
...
# DN trades dual-pass champions
DEFAULT_CELLS = (
    {"name": "imp_t0.008_lb120_tp0.2_sl0.2", "thr": 0.008, "lb": 120, "tp": 0.20, "sl": 0.20},
    {"name": "imp_t0.005_lb60_tp0.2_sl0.2", "thr": 0.005, "lb": 60, "tp": 0.20, "sl": 0.20},
    {"name": "imp_t0.005_lb60_tp0.25_sl0.15", "thr": 0.005, "lb": 60, "tp": 0.25, "sl": 0.15},
)
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--tag", default="research_impulse_scout_dn_quote_dual")
    ...
    lb_need = sorted({int(c["lb"]) for c in DEFAULT_CELLS})
    thr_need = sorted({float(c["thr"]) for c in DEFAULT_CELLS})
    print(
        f"impulse DN scout QUOTE dual {start_all}..{end_all} "
    ...
    for di, date in enumerate(dates):
        ...
        for sym in symbols:
            ...
            for sess_name, s0, s1 in SESSIONS:
                for lb in lb_need:
                    t0 = pd.Timestamp(f"{date} {s0}:00", tz=NY) + pd.Timedelta(seconds=lb)
                    t1 = pd.Timestamp(f"{date} {s1}:00", tz=NY)
                    fired: set[tuple[float, int]] = set()
                    t = t0
    ...
    for cell in DEFAULT_CELLS:
        thr, lb = float(cell["thr"]), int(cell["lb"])
        ...
        for max_sp in spreads:
            for max_lag in lags:
                ...
                for arm in arms:
                    if float(arm["thr"]) != thr or int(arm["lookback_sec"]) != lb:
                        continue
    ...
    summary = {
        ...
        "isolation": "independent scout; not Mag7 Rule-A",
    }
