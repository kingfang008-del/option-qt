...
def fine_tune():
    """
    [New] 微调专用模式 (Fine-Tuning Mode)
    功能:
        1. 加载现有的最佳权重 (继承智商)。
        2. 重置优化器和调度器 (重置体力)。
        3. 使用更低的学习率和较短的 Epoch 进行精细训练。
    """
    # ================= 1. 配置与路径 (参考 main) =================
    ...
    # ================= 2. 准备数据 (使用 UnifiedLMDBDataset) =================
    # 使用 load_meta_info 获取 ID 范围 (保持与 main 一致)
    meta = load_meta_info()
    caps = {'stock': meta['max_stock_id'], 'sector': meta['max_sector_id'], 'dow': 7}
    
    logger.info("🎨 [Fine-Tune] Loading Datasets...")
    # [关键] 必须使用 UnifiedLMDBDataset 以支持 Stock/Option 双流结构

    # 2. [修改] 加载并合并多个验证集
    # 定义你的验证集文件名列表
    val_filenames = [
        # 'val1_slow_channel_alpha.lmdb',
        'val_alpha.lmdb' 
        # 你可以在这里添加更多
    ]
    ...
    train_ds = UnifiedLMDBDataset(str(h5_root/'train_alpha.lmdb'), config)
     
    
    # 保持与 main 一致的 DataLoader 参数
    ...
    if best_ckpt_path.exists():
        logger.info(f"🔄 Loading Best Weights: {best_ckpt_path}")
        ...
        logger.info(f"✅ Weights Loaded. Previous Best IC: {best_ic:.4f}")
        logger.info("   -> Optimizer & Scheduler have been RESET for fine-tuning.")
    else:
        logger.warning("⚠️ No Checkpoint found! Starting from scratch.")
        best_ic = -1.0

    # ================= 5. 微调专用配置 =================
    FT_EPOCHS = 10         # 微调轮数
    FT_LR = 1e-5           # 微调学习率 (main 中是 1e-4，这里降低 5 倍)
    
    # 初始化全新优化器
    optim = AdamW(model.parameters(), lr=FT_LR, weight_decay=1e-3)
    crit = StrategicAlphaLoss().to(device)
    
    # 初始化全新调度器 (基于 FT_EPOCHS)
    # max_lr 设置为 FT_LR 的 5 倍 (OneCycleLR 推荐配置)
    ...
    for ep in range(FT_EPOCHS):
        ...
        for b in pbar:
            ...
            # [关键] 解包逻辑需与 main 中的 collate_fn 返回值一致
            x_stk, x_opt, s, t, _ = b
        ...
        # 保存为 _ft 后缀，不覆盖原始 best
        torch.save(ft_state, ckpt_dir / "advanced_alpha_ft_latest.pth")
        
        if is_best:
            torch.save(ft_state, ckpt_dir / "advanced_alpha_ft_best.pth")
            
    logger.info(f"✅ Fine-Tuning Complete. Best IC: {best_ic:.4f}")

def main():
    config_path = Path.home() / 'notebook/train/slow_feature.json'
    with open(config_path) as f: config = json.load(f)
    h5 = Path('/mnt/s990/data/ret_cost')
    ckpt_dir = Path("./checkpoints_ret_cost")
    ...
    train_dl = DataLoader(UnifiedLMDBDataset(str(h5/'train_ret_cost_alpha.lmdb'), config), batch_size=1024, shuffle=True, collate_fn=collate_fn, num_workers=8, pin_memory=True)
    val_dl = DataLoader(val_ds, batch_size=1024, shuffle=False, collate_fn=collate_fn, num_workers=4)
