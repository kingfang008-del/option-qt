...
# 输出目录：用于存放计算好的特征文件. 
OPTION_MONTHLY_DIR = Path.home() / "train_data/nq_options_monthly_iv"
AGG_OPTION_MONTHLY_DIR = Path.home() / "train_data/nq_options_bucketed_v7"

OUTPUT_FEATURES_DIR = Path.home() / "train_data/features_raw"
# VIX数据路径模板
VIX_PATH_TEMPLATE = str(Path.home() / "data/VIXY_resampled_1min/{year_month}.parquet")
# 配置文件路径
...
def generate_vix_level_global(config: dict):
    """
    【混合标准化恢复版】合并所有VIXY月份数据，一次性计算raw vix_level，然后全局混合标准化，按月切分回写。
    解决相邻行差异大问题，同时保持零值解决。
    """
    input_dir = Path.home() / "data/VIXY_resampled_1min"
    all_files = sorted(input_dir.glob('*.parquet'))
    if not all_files:
        logging.warning("没有找到VIXY Parquet文件。")
        return

    df_all = pd.concat([pd.read_parquet(f) for f in all_files], ignore_index=True)
    if df_all.empty:
        logging.warning("所有VIXY文件为空，无法计算。")
        return
    ...
    # --- 1. 计算去趋势化的绝对水平指标 (不变) ---
    long_term_window = 63 * 390
    long_term_ma = df_all['vix_proxy_close'].rolling(window=long_term_window, min_periods=int(long_term_window/3)).mean()
    df_all['vixy_detrended_level'] = df_all['vix_proxy_close'] / (long_term_ma + 1e-9)
    
    # --- 2. 【核心修改】计算多尺度的 VIX Z-Score ---
    # a. 宏观Z-Score (vix_z)，使用长窗口
    macro_window = 63 * 390 # 约3个月
    macro_min_p = 21 * 390  # 约1个月
    vixy_mean_macro = df_all['vix_proxy_close'].rolling(window=macro_window, min_periods=macro_min_p).mean()
    vixy_std_macro = df_all['vix_proxy_close'].rolling(window=macro_window, min_periods=macro_min_p).std()
    df_all['vix_z'] = (df_all['vix_proxy_close'] - vixy_mean_macro) / (vixy_std_macro + 1e-9)

    # b. 日内Z-Score (vix_level)，使用短窗口（例如60分钟）
    intraday_window = 60 # 过去一小时
    intraday_min_p = 20  # 至少20分钟
    vixy_mean_intraday = df_all['vix_proxy_close'].rolling(window=intraday_window, min_periods=intraday_min_p).mean()
    vixy_std_intraday = df_all['vix_proxy_close'].rolling(window=intraday_window, min_periods=intraday_min_p).std()
    ...
    df_all['vix_level'] = df_all['vix_level'].replace(0, np.nan)  # 临时标记零为NaN
    df_all['vix_level'] = df_all['vix_level'].interpolate(method='linear', limit_direction='both').fillna(method='ffill').fillna(method='bfill')
    logging.info("Raw calculation finished.")   


    df_all['close']=df_all['vix_proxy_close']  # 为了兼容旧代码

    # 5. 按月切分并保存
    df_all['year_month'] = df_all.index.strftime('%Y-%m')
    for year_month, df_month in tqdm(df_all.groupby('year_month'), desc="Saving updated monthly files"):
        file_path = input_dir / f"{year_month}.parquet"
