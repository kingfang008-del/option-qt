#!/usr/bin/env bash
# V4 × 2026-04：从 dte1_options_old_lock 重建特征并 replay
#
# 锁约：locked_targets_map_old_style_trading_1dte.parquet
# 1s： /mnt/s990/data/raw_1s/dte1_options_old_lock
#
# 用法:
#   bash qqq_btc/tools/replay_v4_april2026_old_lock.sh
set -euo pipefail
...
EXP="${EXP:-$HOME/train_data/april_v4_old_lock}"
RAW1S="${RAW1S:-/mnt/s990/data/raw_1s/dte1_options_old_lock}"
OPT1M="${OPT1M:-$EXP/options_1m_april}"
...
OUT="${OUT:-qqq_btc/results/v4_april2026_old_lock}"
...
log "=== V4 April old_lock pipeline start ==="
...
"$PY" preprocess/download/step3_databento_aggregate_1s_to_1m.py \
  ...
  --date-to 2026-04-30 \
  --force
ls "$OPT1M/QQQ"/QQQ_2026-04-*.parquet | wc -l
...
raw = Path("$EXP/quote_options_monthly_iv/QQQ/standard/2026-04.parquet")
out = Path("$EXP/quote_options_bucketed_v7")
print(process_single_file((raw, out, "QQQ")) or "bucketed ok")
print("bucketed", list(out.rglob("2026-04.parquet")))
PY

log "[4] feature_merge QQQ 2026-04"
"$PY" - <<'PY'
import json
from pathlib import Path
import preprocess.ask_bid.feature_merge_option_raw as fm

exp = Path.home() / "train_data/april_v4_old_lock"
...
with open(cfg_path) as f:
    config = json.load(f)
print(fm.process_stock_month("QQQ", "2026-04", config))
for res in ("1min", "5min"):
    p = fm.OUTPUT_FEATURES_DIR / f"QQQ/regular/09:30-16:00/{res}/2026-04.parquet"
    import pandas as pd
    print(res, "exists", p.exists(), "rows", pd.read_parquet(p).shape[0] if p.exists() else 0)
PY

log "[5] assemble + rolling_norm + label (March bak warmstart)"
...
if [[ -f "$BAK1/1min/2026-03.parquet" ]]; then
  cp "$BAK1/1min/2026-03.parquet" "$TEST1/"
  cp "$BAK1/5min/2026-03.parquet" "$TEST5/" 2>/dev/null || true
fi
cp "$EXP/quote_features_raw/QQQ/regular/09:30-16:00/1min/2026-04.parquet" "$TEST1/"
cp "$EXP/quote_features_raw/QQQ/regular/09:30-16:00/5min/2026-04.parquet" "$TEST5/" 2>/dev/null || true

"$PY" - <<'PY'
import os
from pathlib import Path
import preprocess.ask_bid.apply_rolling_norm_standalone as norm
exp = Path.home() / "train_data/april_v4_old_lock/quote_features_test/QQQ/regular/09:30-16:00/1min"
...
# 标签必须用本链 day_iv + old_lock map。默认 anchor_qqq_1dte 指向
# quote_options_day_iv_dte1(四月仅有 4/13+),会导致前半月 exec_* 全空。
...
cfg["paths"] = {
    ...
    "locked_targets_output": "$LOCK_MAP",
}
out = Path("$ANCHOR_OL")
out.write_text(json.dumps(cfg, indent=2, ensure_ascii=False) + "\n")
print("anchor_label", out)
...
"$PY" qqq_btc/tools/label_pipeline.py \
  ...
  --output "$TEST1" \
  --symbol QQQ \
  --anchor-config "$ANCHOR_OL" \
  --report "$EXP/label_report_april_old_lock.json"

log "[6] V4 infer April-only + live-replay"
TMP="$EXP/eval_feat_april_only"
rm -rf "$TMP"
mkdir -p "$TMP/QQQ/regular/09:30-16:00/1min" "$TMP/QQQ/regular/09:30-16:00/5min"
cp "$TEST1/2026-04.parquet" "$TMP/QQQ/regular/09:30-16:00/1min/"
cp "$TEST5/2026-04.parquet" "$TMP/QQQ/regular/09:30-16:00/5min/" 2>/dev/null || true
...
raw1 = pd.read_parquet(
    EXP / "quote_features_raw/QQQ/regular/09:30-16:00/1min/2026-04.parquet",
...
trades.to_parquet(OUT / "v4_april2026_old_lock_trades.parquet", index=False)
...
out = {
    ...
    "recipe": "old_lock_features + matched day_iv labels + 1min_causal(+1m) put_gate + LIVE_REPLAY + q10 on CALL/PUT + start_bar=60",
    "paths": {
        "ckpt": os.environ["CKPT"],
        "raw_1s": os.environ["RAW1S"],
        "option_1m": os.environ["OPT1M"],
        "lock_map": os.environ["LOCK_MAP"],
        "feat_root": str(EXP / "eval_feat_april_only"),
        "infer": str(EVAL / "test_infer.parquet"),
        "anchor_label": str(EXP / "anchor_old_lock_label.json"),
    },
    "gates": {
        "edge_q10_floor": q10,
        "q10_applies_to": ["CALL", "PUT"],
...
print()
print(
    f"V4 April old_lock honest KPI: acct25={r['acct25']*100:+.2f}% "
...
if out["eval_builtin_live_feat_vix"].get("acct25") is not None:
    ...
    print(
        f"V4 April old_lock builtin LIVE(feat vix): acct25={b['acct25']*100:+.2f}% "
        f"trades={b['trades']}"
    )
PY
