...
from strategy.entry_risk_rules import get_entry_min_option_price
from strategy.liquidity_rules import LiquidityRiskManager
from oms.runtime_trading_controls import (
...
class OrchestratorExecution:
    ...
    async def _execute_entry(self, sym, sig, stock_price, curr_ts, batch_idx):
        ...
        st.max_roi = 0.0
        st.exec_mode = str(_meta.get('exec_mode', sig.get('exec_mode', 'SWING')) or 'SWING')
        st.exec_dte = _mi(_meta.get('exec_dte', sig.get('exec_dte', 1)), 1)
        st.exec_profile = str(_meta.get('exec_profile', '') or '')
        st.hold_profile = str(_meta.get('hold_profile', '') or '')
        st.exec_band = str(_meta.get('exec_band', sig.get('exec_band', '')) or '')
        st.band_legs_today = int(getattr(st, 'band_legs_today', 0) or 0) + (
            1 if st.exec_profile == 'multi_band' and st.exec_band else 0
        )
        st.init_ctx = {
            'alpha_z': st.entry_alpha_z,
            'net_edge_raw': st.entry_alpha_z,
            'spy_roc': st.entry_spy_roc,
            'index_trend': st.entry_index_trend,
            'exec_mode': st.exec_mode,
            'exec_dte': st.exec_dte,
