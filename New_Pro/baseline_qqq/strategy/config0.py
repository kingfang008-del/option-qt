...
class StrategyConfig:
    ...
    # ================= 2. Trading Session =================
    # 默认 09:30 开仓（与 qqq_btc replay session_bar=0 对齐）；legacy 09:45 设 START_MINUTE=45
    try:
        from config import START_MINUTE as _CFG_START_MIN
    except Exception:  # pragma: no cover
        _CFG_START_MIN = 30
    START_HOUR: int = 9
    START_MINUTE: int = int(_CFG_START_MIN)
    START_TIME: str = f"09:{int(_CFG_START_MIN):02d}:00"
    ...
    # qqq_btc live 下由 bootstrap 设 FAST_GATE_ENABLED=0，spread 仅走 choose_entry 6%
    try:
        from config import FAST_GATE_ENABLED as _CFG_FAST_GATE
    except Exception:  # pragma: no cover
        _CFG_FAST_GATE = True
    FAST_GATE_ENABLED: bool = bool(_CFG_FAST_GATE)
    ...
    # ================= 4b. Trend Hunter Core [DEPRECATED — STRATEGY_CORE_VERSION=TREND 已归档] =================
    # 以下参数在 QQQ_BTC_LIVE + entry_bridge 路径下不参与 live 决策；保留供 Dashboard 快照兼容。
    ...
    try:
        from config import COOLDOWN_MINUTES as _CFG_COOLDOWN_MIN
    except Exception:  # pragma: no cover
        _CFG_COOLDOWN_MIN = 60
    COOLDOWN_MINUTES: int = int(_CFG_COOLDOWN_MIN)
    ...
    try:
        from config import BIDIRECTIONAL_DISLOCATION_ENTRY_ENABLED as _CFG_DISLOC
    except Exception:  # pragma: no cover
        _CFG_DISLOC = False
    BIDIRECTIONAL_DISLOCATION_ENTRY_ENABLED: bool = bool(_CFG_DISLOC)
