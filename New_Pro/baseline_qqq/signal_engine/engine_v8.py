...
描述: [V8 AlphaEngine - QQQ Net-Edge 专版]
职责:
    - 特征抽取 + TFT 推理 → net_edge（绝对值，无截面排序）
    - 快通道 vol_z / spread / IV 动量门控
    - Alpha 日志落库 + ALPHA_FRAME 下发 OMS
...
_BASELINE = _PKG.parent
_NEW_PRO = _BASELINE.parent
MODEL_DIR = str(_NEW_PRO / "model")
DAO_DIR = str(_BASELINE / "DAO")
for _p in (MODEL_DIR, DAO_DIR):
    if _p not in sys.path:
        sys.path.insert(0, _p)

# 引入纯策略核心
from strategy.selector import ACTIVE_STRATEGY_CORE_VERSION, StrategyConfig

# [Refactor] 引入模块化执行组件
# AlphaEngine 只保留 state 持久化 + ALPHA 日志落库, 策略/交易已下沉到 OMS (execution_engine_v8).
from oms.orchestrator_state_manager import OrchestratorStateManager
from oms.orchestrator_accounting import OrchestratorAccounting
...
# 动态引入组件
try:
    from infra.mock_ibkr_historical import MockIBKRHistorical
except ImportError:
...
from config import (
    ...
    ALPHA_ZSCORE_MODE,
    NET_EDGE_CLIP,
    VOL_Z_USE_PRICE_PROXY,
    BIDIRECTIONAL_DUAL_EDGE_ENABLED,
    BIDIRECTIONAL_DUAL_EDGE_THRESHOLD,
)
from strategy.regime import dual_edges_from_model_out, pick_tradable_side
from signal_engine.alpha_normalization import normalize_alpha_scores
...
class SignalEngineV8:
    def __init__(self, symbols, mode='realtime', config_paths=None, model_paths=None, ibkr=None, shared_signal_queue=None):
        ...
        self.cached_alphas = {}
        self.cached_net_edges = {}
        self.cached_call_edges = {}
        self.cached_put_edges = {}
        self.cached_exec_costs = {}
    ...
    def _prep_symbol_metrics(self, i, sym, stock_prices, raw_alphas, opt_data, metrics_batch, use_precalc_feed=False, metrics_batch_manual=None):
        ...
        if use_precalc_feed:
            ...
            cross_z = alpha_z
        elif str(ALPHA_ZSCORE_MODE).lower() in {'absolute', 'raw', 'net_edge'}:
            alpha_z = float(max(-NET_EDGE_CLIP, min(NET_EDGE_CLIP, raw_alpha_val)))
            cross_z = alpha_z
            vol_z = float(vol_z_dict.get(sym, raw_vols[i] if i < len(raw_vols) else 0.0))
        else:
        ...
        metrics = {
            ...
            'net_edge_raw': float(raw_alpha_val),
            'execution_cost_pred': float(self.cached_exec_costs.get(sym, 0.0)),
            'options_vw_spread': float((metrics_batch.get('options_vw_spread') or {}).get(sym, 0.0)),
            'options_iv_momentum': float((metrics_batch.get('options_iv_momentum') or {}).get(sym, 0.0)),
            'st': st
        }
        return metrics

    def _extract_slow_model_scores(self, model_out: dict) -> dict:
        """从 TFT 输出中提取 QQQ 交易用分数（Phase 3: 优先双 edge argmax）。"""
        ...
        if BIDIRECTIONAL_DUAL_EDGE_ENABLED and USE_NET_EDGE_ALPHA and 'net_edge' in model_out:
            try:
                ...
                th = float(BIDIRECTIONAL_DUAL_EDGE_THRESHOLD)
                for i in range(len(call_edges)):
                    direction, signed, _ = pick_tradable_side(
        ...
        if call_edges is None:
            if USE_NET_EDGE_ALPHA and 'net_edge' in model_out:
                edge = model_out['net_edge'].detach().cpu().numpy().reshape(-1)
            elif 'rank_score' in model_out:
                edge = model_out['rank_score'].detach().cpu().numpy().reshape(-1)
            else:
                edge = model_out.get('logits_dir')
                if edge is not None:
                    logits = edge.detach().cpu().numpy()
                    edge = logits.argmax(axis=-1).astype(np.float32) - 1.0
                else:
    ...
    async def _run_model_inference(self, batch, symbols, prices, ny_now):
        ...
        with torch.no_grad():
            ...
            scores = self._extract_slow_model_scores(out)
            preds = scores['edge']
            exec_costs = scores['execution_cost']
            call_edges = scores.get('call_edge')
            put_edges = scores.get('put_edge')
            for i_p, s_p in enumerate(symbols):
                self.cached_alphas[s_p] = float(preds[i_p])
                self.cached_net_edges[s_p] = float(preds[i_p])
                self.cached_exec_costs[s_p] = float(exec_costs[i_p] if i_p < len(exec_costs) else 0.0)
                if call_edges is not None and put_edges is not None:
                    self.cached_call_edges[s_p] = float(call_edges[i_p])
                    self.cached_put_edges[s_p] = float(put_edges[i_p])
    ...
    def _update_volatility_metrics(self, batch, symbols):
        ...
        use_price_proxy_env = os.environ.get('VOL_Z_USE_PRICE_PROXY', '1' if VOL_Z_USE_PRICE_PROXY else '0').strip().lower()
    ...
    async def process_batch(self, batch: dict):
        ...
        batch['spy_day_roc'] = spy_day_roc
        batch['qqq_day_roc'] = qqq_day_roc
        ...
        # 7. Alpha 归一化：QQQ absolute(net_edge) 不做截面；legacy 才走 z-score
        raw_alphas = np.asarray(raw_alphas, dtype=np.float64)
        _abs_mode = str(ALPHA_ZSCORE_MODE).lower() in {'absolute', 'raw', 'net_edge'}
        alpha_z_map, alpha_norm_stats = normalize_alpha_scores(
            ALPHA_ZSCORE_MODE,
            symbols,
            raw_alphas.tolist(),
            exclude_symbols=self.alpha_norm_exclude_symbols,
            clip=NET_EDGE_CLIP if _abs_mode else 5.0,
        )
        if _abs_mode:
            mean_a = float(np.mean(raw_alphas))
            std_a = float(np.std(raw_alphas) or 1.0)
            alpha_z_map = {
                sym: float(max(-NET_EDGE_CLIP, min(NET_EDGE_CLIP, raw_alphas[i])))
                for i, sym in enumerate(symbols)
                if sym not in self.non_tradable_symbols
            }
        else:
            mean_a = float(alpha_norm_stats.get('global_stats', {}).get('mean', np.mean(raw_alphas)))
            std_a = float(alpha_norm_stats.get('global_stats', {}).get('std', np.std(raw_alphas) or 1.0))

        batch['alpha_mean'] = float(mean_a)
        batch['alpha_std'] = float(std_a if std_a > 1e-5 else 1.0)
        batch['alpha_z_map'] = alpha_z_map
        ...
        spread_map = {}
        iv_mom_map = {}
        spread_arr = batch.get('options_vw_spread')
        iv_mom_arr = batch.get('options_iv_momentum')
        if isinstance(spread_arr, (list, np.ndarray)):
            for idx_s, sym_s in enumerate(symbols):
                if idx_s < len(spread_arr):
                    spread_map[sym_s] = float(spread_arr[idx_s])
        if isinstance(iv_mom_arr, (list, np.ndarray)):
            for idx_s, sym_s in enumerate(symbols):
                if idx_s < len(iv_mom_arr):
                    iv_mom_map[sym_s] = float(iv_mom_arr[idx_s])
        ...
        metrics_batch = {
            ...
            'curr_ts': curr_ts,
            'fast_vol': raw_vols,
            'options_vw_spread': spread_map,
            'options_iv_momentum': iv_mom_map,
        ...
        for i, sym in enumerate(symbols):
            ...
            if sym == 'QQQ':
                raw_alpha_val = float(raw_alphas[i])
                logger.info(
                    f"🔍 [NetEdge-Audit] {sym} | raw={raw_alpha_val:.6f} | "
                    f"mode={ALPHA_ZSCORE_MODE} | cost={self.cached_exec_costs.get(sym, 0.0):.6f}"
                )
            ...
            alpha_items.append({
                ...
                'alpha': metrics['final_alpha'],
                'call_edge': float(self.cached_call_edges.get(sym, 0.0)),
                'put_edge': float(self.cached_put_edges.get(sym, 0.0)),
                'qqq_day_roc': float(metrics_batch.get('qqq_day_roc', 0.0)),
                'spy_day_roc': float(metrics_batch.get('spy_day_roc', 0.0)),
                'cs_alpha_z': metrics['cs_alpha_z'],
                'net_edge_raw': metrics.get('net_edge_raw', metrics['final_alpha']),
                'execution_cost_pred': metrics.get('execution_cost_pred', 0.0),
                'options_vw_spread': metrics.get('options_vw_spread', 0.0),
                'options_iv_momentum': metrics.get('options_iv_momentum', 0.0),
                'vol_z': metrics['vol_z'],
        ...
        if alpha_items and not getattr(self, 'only_log_alpha', False):
            await self._publish_alpha_frame(
                ...
                spy_rocs=spy_rocs,
                qqq_rocs=qqq_rocs,
                spy_day_roc=spy_day_roc,
                qqq_day_roc=qqq_day_roc,
    ...
    async def _publish_alpha_frame(
        ...
        payload = {
            ...
            'spy_day_roc': float(spy_day_roc or 0.0),
            'qqq_day_roc': float(qqq_day_roc or 0.0),
        }
        if getattr(self, 'use_shared_mem', False):
