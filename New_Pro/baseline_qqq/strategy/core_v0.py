...
from strategy.entry_risk_rules import evaluate_entry_liquidity
from strategy.regime import enrich_ctx_regime
from strategy.exec_profile import effective_exit_param
...
GATE_REGISTRY = {
    ...
    "E8.simple_routes":      "极简双路径路由",
    "E8.trend_vol":          "趋势腿 VOL_Z",
    "E8.trend_alpha":        "趋势腿 Alpha 门槛",
    "E8.trend_momentum":     "趋势腿动量(极简跳过)",
    "E8.trend_snap_spike":   "趋势腿尖刺拦截",
    "E8.trend_index":        "趋势腿大盘同向",
    "E8.trend_macd":         "趋势腿 MACD(极简跳过)",
    "E8.ch_b_stock_momentum": "慢牛个股顺势 stock_roc > 0.0005",
    "E8.ch_b_snap_roc":      "慢牛瞬时动量 snap_roc",
    "E0.dislocation_band1":  "Multi-Band 错价快打 (BAND1 dislocation)",
...
class StrategyCoreV0:
    ...
    def _xp(self, pos: dict, name: str):
        """Profile-aware exit param (SCALP vs SWING overlay)."""
        return effective_exit_param(self.cfg, pos, name)
    ...
    def _alpha_signal(self, ctx: dict) -> float:
        """QQQ absolute 模式：net_edge；legacy 下 alpha_z 仍为截面 z。"""
        raw = ctx.get('net_edge_raw')
        if raw is not None:
            try:
                return float(raw)
            except (TypeError, ValueError):
                pass
        return float(ctx.get('alpha_z', 0.0) or 0.0)
    ...
    def decide_entry(self, ctx: dict) -> dict:
        """
        [开仓决策 - 核心路由入口]
        V0_SIMPLE_ENTRY: 错价腿 → 趋势腿（两道）；否则 legacy Ch-B / Ch-A 全量门禁。
        """
        self._last_reject_reason = None
        ...
        if getattr(self.cfg, 'V0_SIMPLE_ENTRY_ENABLED', True):
            sig = self._decide_entry_simple(ctx)
        else:
            sig = self._decide_entry_legacy(ctx)

        if not sig:
            if not self._last_reject_reason:
                self._last_reject_reason = 'no_entry_path'
            return None

        return self._finalize_entry_signal(ctx, sig)

    def _decide_entry_simple(self, ctx: dict) -> dict | None:
        """极简栈：Path1 错价 → Path2 趋势（alpha + 大盘同向）。"""
        sig = None
        if self._is_multi_band_profile(ctx):
            sig = self._try_dislocation_band1_entry(ctx)
        elif getattr(self.cfg, 'BIDIRECTIONAL_DISLOCATION_ENTRY_ENABLED', True):
            sig = self._try_dislocation_entry(ctx)

        if not sig:
            sig = self._try_trend_entry(ctx)

        if not sig:
            self._trace("E8.simple_routes", "block", "dislocation+trend both miss")
        else:
            self._trace("E8.simple_routes", "pass", sig.get('reason', ''))
        return sig

    def _decide_entry_legacy(self, ctx: dict) -> dict | None:
        """Legacy：Ch-B 慢牛 + Ch-A 全量动量/MACD/INDEX 门禁。"""
        sig = None
        if self._is_multi_band_profile(ctx):
            sig = self._try_dislocation_band1_entry(ctx)
        elif getattr(self.cfg, 'BIDIRECTIONAL_DISLOCATION_ENTRY_ENABLED', True):
    ...
    def _try_trend_entry(self, ctx: dict) -> dict | None:
        """趋势腿：|alpha| + vol 窗 + 大盘同向；极简模式跳过 MACD/5m 动量套件。"""
        vol_z = ctx['vol_z']
        alpha_val = self._alpha_signal(ctx)
        action = 1 if alpha_val > 0 else -1

        if not (self.cfg.VOL_MIN_Z < vol_z < self.cfg.VOL_MAX_Z):
            self._trace("E8.trend_vol", "block",
                        f"vol_z={vol_z:.2f} ∉ ({self.cfg.VOL_MIN_Z}, {self.cfg.VOL_MAX_Z})")
            return None
        self._trace("E8.trend_vol", "pass", f"vol_z={vol_z:.2f}")

        symbol = ctx.get('symbol', 'UNKNOWN')
        dynamic_threshold = self._calculate_dynamic_alpha_threshold(symbol, vol_z, action, ctx)
        if abs(alpha_val) < dynamic_threshold:
            self._trace("E8.trend_alpha", "block",
                        f"|alpha|={abs(alpha_val):.3f} < {dynamic_threshold:.3f}")
            return None
        self._trace("E8.trend_alpha", "pass",
                    f"|alpha|={abs(alpha_val):.3f} ≥ {dynamic_threshold:.3f}")

        skip_mom = getattr(self.cfg, 'SIMPLE_TREND_SKIP_MOMENTUM', True)
        if not skip_mom and self._cfg_enabled('ENTRY_MOMENTUM_GUARD_ENABLED', True):
            if not self._check_stock_momentum_guard(ctx, action):
                return None
        else:
            snap_roc = float(ctx.get('snap_roc', 0.0) or 0.0)
            if abs(snap_roc) > self.cfg.MAX_SNAP_ROC_LIMIT:
                self._trace("E8.trend_snap_spike", "block",
                            f"|snap|={abs(snap_roc):.4f} > {self.cfg.MAX_SNAP_ROC_LIMIT}")
                return None
            self._trace("E8.trend_momentum", "skip", "SIMPLE_TREND_SKIP_MOMENTUM=True")

        if not self._check_simple_index_align(ctx, action):
            return None

        if not getattr(self.cfg, 'SIMPLE_TREND_SKIP_MACD', True):
            if not self._check_macd_confirm(ctx, action):
                return None
        else:
            self._trace("E8.trend_macd", "skip", "SIMPLE_TREND_SKIP_MACD=True")
        ...
        return {
            ...
            'score': abs(alpha_val),
            'reason': f"CH_TREND|A:{alpha_val:.3f}(Th:{dynamic_threshold:.3f})|V:{vol_z:.1f}",
        }

    def _check_simple_index_align(self, ctx: dict, action: int) -> bool:
        """趋势腿唯一大盘护栏：call 需指数非弱，put 需指数非强（跌日放宽）。"""
        spy = float(ctx.get('spy_roc', 0.0) or 0.0)
        qqq = float(ctx.get('qqq_roc', 0.0) or 0.0)
        min_idx = float(getattr(self.cfg, 'SIMPLE_TREND_MIN_INDEX_ROC', 0.00015))
        day_type = str(ctx.get('day_type', '')).lower()

        if action >= 0:
            if spy < -min_idx and qqq < -min_idx:
                self._trace("E8.trend_index", "block",
                            f"dir=+1 index weak spy={spy:.4f} qqq={qqq:.4f}")
                return False
        else:
            if getattr(self.cfg, 'BIDIRECTIONAL_INDEX_GUARD_ENABLED', True) and day_type == 'trend_down':
                block_th = float(getattr(self.cfg, 'BIDIRECTIONAL_PUT_BLOCK_INDEX_ROC', 0.0015))
                if spy > block_th and qqq > block_th:
                    self._trace("E8.trend_index", "block",
                                f"dir=-1 rally on bear day spy={spy:.4f} qqq={qqq:.4f}")
                    return False
            elif spy > min_idx and qqq > min_idx:
                self._trace("E8.trend_index", "block",
                            f"dir=-1 index strong spy={spy:.4f} qqq={qqq:.4f}")
                return False

        self._trace("E8.trend_index", "pass", f"dir={action} spy={spy:.4f} qqq={qqq:.4f}")
        return True

    def _finalize_entry_signal(self, ctx: dict, sig: dict) -> dict | None:
        """流动性与点差拦截（simple / legacy 共用）。"""
        if self._cfg_enabled('ENTRY_LIQUIDITY_GUARD_ENABLED', True):
            spread_override = None
            reason = str(sig.get('reason', ''))
            if reason.startswith('CH_DISLOC'):
                spread_override = float(getattr(self.cfg, 'MULTI_BAND1_MAX_SPREAD', 0.18))
                if 'CH_DISLOC_PUT' in reason or sig.get('dir', 0) < 0:
                    spread_override = max(
                        spread_override,
                        float(getattr(self.cfg, 'BIDIRECTIONAL_DISLOC_MAX_SPREAD', 0.18)),
                    )
            if not self._check_entry_liquidity_guard(ctx, spread_threshold_override=spread_override):
    ...
    def _check_entry_pre_conditions(self, ctx: dict) -> bool:
        ...
        if getattr(self.cfg, 'FAST_GATE_ENABLED', False):
            if not self._check_fast_regime_gate(ctx):
                return False

        return True

    def _check_fast_regime_gate(self, ctx: dict) -> bool:
        """快通道：期权 spread / IV 动量门控（非 alpha，仅过滤不可交易环境）。"""
        spread = float(ctx.get('options_vw_spread', 0.0) or 0.0)
        iv_mom = float(ctx.get('options_iv_momentum', 0.0) or 0.0)
        spread_max = float(getattr(self.cfg, 'FAST_GATE_SPREAD_MAX', 0.12))
        iv_mom_max = float(getattr(self.cfg, 'FAST_GATE_IV_MOMENTUM_ABS_MAX', 0.50))
        if spread > spread_max > 0:
            self._trace("E6b.fast_spread", "block", f"spread={spread:.4f} > {spread_max:.4f}")
            return False
        if abs(iv_mom) > iv_mom_max > 0:
            self._trace("E6b.fast_iv_mom", "block", f"|iv_mom|={abs(iv_mom):.4f} > {iv_mom_max:.4f}")
            return False
        self._trace("E6b.fast_gate", "pass", f"spread={spread:.4f}, iv_mom={iv_mom:.4f}")
        return True
    ...
    def _is_multi_band_profile(self, ctx: dict) -> bool:
        prof = str(ctx.get('exec_profile', '') or '').strip().lower()
        return prof in ('multi_band', 'bands', 'roll')

    def _try_dislocation_band1_entry(self, ctx: dict) -> dict | None:
        """BAND1 错价腿：委托通用 dislocation 逻辑。"""
        return self._try_dislocation_entry(ctx, band1_only=True)

    def _try_dislocation_entry(self, ctx: dict, *, band1_only: bool = False) -> dict | None:
        """错价腿：低价 + snap 反弹/回落，不要求 5m stock_roc / MACD（call/put 对称）。"""
        if band1_only and not getattr(self.cfg, 'MULTI_BAND_DISLOCATION_ENTRY_ENABLED', True):
            self._trace("E0.dislocation", "skip", "MULTI_BAND_DISLOCATION_ENTRY_ENABLED=False")
            return None
        if not band1_only and not getattr(self.cfg, 'BIDIRECTIONAL_DISLOCATION_ENTRY_ENABLED', True):
            self._trace("E0.dislocation", "skip", "BIDIRECTIONAL_DISLOCATION_ENTRY_ENABLED=False")
            return None

        curr_price = float(ctx.get('curr_price', 0.0) or 0.0)
        if band1_only:
            price_ceiling = float(getattr(self.cfg, 'MULTI_BAND2_PRICE_FLOOR', 0.85))
        else:
            price_ceiling = float(getattr(self.cfg, 'BIDIRECTIONAL_DISLOC_MAX_PRICE', 0.85))
        if curr_price <= 0.01 or curr_price >= price_ceiling:
            self._trace("E0.dislocation", "skip", f"price={curr_price:.2f} not in disloc band")
            return None

        alpha_val = self._alpha_signal(ctx)
        action = 1 if alpha_val > 0 else -1
        if band1_only:
            min_edge = float(getattr(self.cfg, 'MULTI_BAND1_MIN_NET_EDGE', 0.012))
            min_snap = float(getattr(self.cfg, 'MULTI_BAND1_MIN_SNAP_ROC', 0.0008))
            max_spread = float(getattr(self.cfg, 'MULTI_BAND1_MAX_SPREAD', 0.18))
        else:
            min_edge = float(getattr(self.cfg, 'BIDIRECTIONAL_DISLOC_MIN_NET_EDGE', 0.012))
            min_snap = float(getattr(self.cfg, 'BIDIRECTIONAL_DISLOC_MIN_SNAP_ROC', 0.0008))
            max_spread = float(getattr(self.cfg, 'BIDIRECTIONAL_DISLOC_MAX_SPREAD', 0.18))

        if abs(alpha_val) < min_edge:
            self._trace("E0.dislocation", "block", f"|alpha|={abs(alpha_val):.3f}<{min_edge:.3f}")
            return None

        snap_roc = float(ctx.get('snap_roc', 0.0) or 0.0)
        if action == 1 and snap_roc < min_snap:
            self._trace("E0.dislocation", "block", f"dir=+1 snap={snap_roc:.4f}<{min_snap:.4f}")
            return None
        if action == -1 and snap_roc > -min_snap:
            self._trace("E0.dislocation", "block", f"dir=-1 snap={snap_roc:.4f}>{-min_snap:.4f}")
            return None

        spread_feat = float(ctx.get('options_vw_spread', 0.0) or 0.0)
        bid = float(ctx.get('bid', 0.0) or 0.0)
        ask = float(ctx.get('ask', 0.0) or 0.0)
        if bid > 0.01 and ask > 0.01 and curr_price > 0.01:
            spread_feat = max(spread_feat, (ask - bid) / curr_price)
        if spread_feat > max_spread > 0:
            self._trace("E0.dislocation", "block", f"spread={spread_feat:.2%}>{max_spread:.0%}")
            return None

        tag_suffix = "PUT" if action < 0 else "CALL"
        reason_tag = f"CH_DISLOC_{tag_suffix}" if action < 0 else "CH_DISLOC"
        self._trace("E0.dislocation", "pass", f"p={curr_price:.2f}|snap={snap_roc:.4f}|a={alpha_val:.3f}")
        return {
            'action': 'BUY',
            'dir': action,
            'tag': option_bucket_tag(action),
            'legacy_tag': option_legacy_tag(action),
            'score': abs(alpha_val),
            'reason': f"{reason_tag}|A:{alpha_val:.3f}|snap:{snap_roc:.4f}|p:{curr_price:.2f}",
        }

    def _check_entry_liquidity_guard(self, ctx: dict, spread_threshold_override: float | None = None) -> bool:
        ...
        decision = evaluate_entry_liquidity(
            ...
            cfg=self.cfg,
            spread_threshold_override=spread_threshold_override,
        )
    ...
    def _calculate_dynamic_alpha_threshold(
        self, symbol: str, vol_z: float, action: int = 0, ctx: dict | None = None,
    ) -> float:
        """根据波动率动态调整 net_edge 门槛（absolute 模式）。"""
        base_threshold = float(self.cfg.ALPHA_ENTRY_THRESHOLD)
        if action < 0 and getattr(self.cfg, 'BIDIRECTIONAL_ENABLED', True):
            day_type = str((ctx or {}).get('day_type', '')).lower()
            if day_type == 'trend_down':
                base_threshold *= float(getattr(self.cfg, 'BIDIRECTIONAL_PUT_ALPHA_RELAX', 0.85))
        dynamic_threshold = base_threshold
        if vol_z > 2.0:
            dynamic_threshold += (vol_z - 2.0) * 0.003
        cap = float(getattr(self.cfg, 'ALPHA_ENTRY_STRICT', base_threshold * 2.0))
        return min(dynamic_threshold, cap)
    ...
    def _check_stock_momentum_guard(self, ctx: dict, action: int) -> bool:
        ...
        if action < 0 and getattr(self.cfg, 'BIDIRECTIONAL_ENABLED', True):
            if str(ctx.get('day_type', '')).lower() == 'trend_down':
                tol *= float(getattr(self.cfg, 'BIDIRECTIONAL_PUT_MOMENTUM_RELAX', 1.5))
        min_snap = self.cfg.MIN_LAST_SNAP_ROC
        if action < 0 and getattr(self.cfg, 'BIDIRECTIONAL_ENABLED', True):
            if str(ctx.get('day_type', '')).lower() == 'trend_down':
                min_snap *= 0.85
    ...
    def _check_index_guard(self, ctx: dict, action: int) -> bool:
        """大盘卫士：涨日拦 put、跌日拦 call；双向模式下跌日 put 更易通过。"""
        ...
        day_type = str(ctx.get('day_type', '')).lower()
        bidir = getattr(self.cfg, 'BIDIRECTIONAL_INDEX_GUARD_ENABLED', True) and getattr(
            self.cfg, 'BIDIRECTIONAL_ENABLED', True,
        )

        if action == -1 and self.cfg.INDEX_GUARD_SHORT_BLOCK_ENABLED:
            if bidir and day_type == 'trend_down':
                block_th = float(getattr(self.cfg, 'BIDIRECTIONAL_PUT_BLOCK_INDEX_ROC', 0.0015))
                if spy_roc > block_th and qqq_roc > block_th:
                    self._trace("E12.ch_a_index_guard", "block",
                                f"dir=-1 跌日但指数急拉 | spy={spy_roc:.4f} qqq={qqq_roc:.4f}")
                    return False
            elif idx_trend == 1 or spy_roc > 0.0001 or qqq_roc > 0.0001:
                self._trace("E12.ch_a_index_guard", "block",
        ...
        if action == 1:
            th = threshold
            if bidir and day_type == 'trend_up':
                th = threshold * 1.25
            if spy_roc < th or qqq_roc < th:
                self._trace("E12.ch_a_index_guard", "block",
                            f"dir=+1 指数跌破 th={th} | spy={spy_roc:.4f} qqq={qqq_roc:.4f}")
                return False

        if action == -1 and bidir and day_type == 'trend_down':
            pass  # 跌日 put：仅上方急拉拦截，不再用全日 ROC 二次挡单
    ...
    def _check_macd_confirm(self, ctx: dict, action: int) -> bool:
        ...
        if action < 0:
            th = float(getattr(self.cfg, 'MACD_HIST_THRESHOLD_PUT', th))
            if getattr(self.cfg, 'BIDIRECTIONAL_ENABLED', True) and str(ctx.get('day_type', '')).lower() == 'trend_down':
                th *= float(getattr(self.cfg, 'BIDIRECTIONAL_PUT_MACD_RELAX', 0.70))
    ...
    def _get_dynamic_ladder(self, pos: dict) -> List[Tuple[float, float]]:
        ...
        init_alpha = abs(pos.get('init_ctx', {}).get('net_edge_raw', pos.get('init_ctx', {}).get('alpha_z', 0.0)))
    ...
    def _evaluate_profit_guards(self, pos: dict, current_roi: float) -> dict:
        ...
        # Level Epic: 暴利追踪（连续回撤带）；multi_band 各档始终启用 band trailing overlay
        use_continuous = getattr(self.cfg, "V0_PROFIT_HYBRID_CONTINUOUS_ENABLED", True)
        if pos.get('exec_band'):
            use_continuous = True
        if use_continuous:
            trail_trig = float(self._xp(pos, "TRAILING_TRIGGER_ROI"))
            trail_keep = float(self._xp(pos, "TRAILING_KEEP_RATIO"))
            if max_roi >= trail_trig:
                trailing_exit = max_roi * trail_keep
                ...
                self._trace("X10.trailing_epic", "skip",
                            f"max_roi={max_roi:.1%} < trigger={trail_trig:.0%}")
        else:
        ...
        flash_trig = float(self._xp(pos, "FLASH_PROTECT_TRIGGER"))
        flash_exit = float(self._xp(pos, "FLASH_PROTECT_EXIT"))
        if max_roi >= flash_trig:
            if current_roi <= flash_exit:
                self._trace("X10.flash_protect", "block",
                            f"max={max_roi:.1%}≥trig={flash_trig:.0%} cur={current_roi:.1%}≤exit={flash_exit:.0%}")
            ...
            self._trace("X10.flash_protect", "skip",
                        f"max_roi={max_roi:.1%} < trigger={flash_trig:.0%}")
    ...
    def _check_time_and_inactivity_stops(self, ctx: dict, pos: dict, held_mins: float, roi: float) -> dict:
        ...
        no_mom_mins = int(self._xp(pos, "NO_MOMENTUM_MINS"))
        no_mom_roi = float(self._xp(pos, "NO_MOMENTUM_MIN_MAX_ROI"))
        if held_mins >= no_mom_mins and roi < no_mom_roi:
            self._trace("X5.no_momentum", "block",
                        f"held={held_mins:.1f}m >= {no_mom_mins}m | roi={roi:.2%} < {no_mom_roi:.2%} | max_roi={max_roi:.2%}")
        ...
        mid_mins = int(self._xp(pos, "MID_TIME_STOP_MINS"))
        mid_roi = float(self._xp(pos, "MID_TIME_STOP_ROI"))
        if held_mins >= mid_mins and roi < mid_roi:
            self._trace("X8.time_stop", "block",
                        f"held={held_mins:.0f}m >= {mid_mins}m | roi={roi:.1%} < {mid_roi:.1%}")
            return {'action': 'SELL', 'reason': f"TIME_STOP15:{held_mins:.0f}m"}

        time_mins = int(self._xp(pos, "TIME_STOP_MINS"))
        time_roi = float(self._xp(pos, "TIME_STOP_ROI"))
        if held_mins > time_mins and roi < time_roi:
            self._trace("X8.time_stop", "block",
                        f"held={held_mins:.0f}m > {time_mins}m | roi={roi:.1%} < {time_roi:.1%}")
    ...
    def _check_stop_loss_guards(self, ctx: dict, pos: dict, roi: float, stock_roi: float) -> dict:
        ...
        stop_loss = float(self._xp(pos, "STOP_LOSS"))
        if roi >= stop_loss:
            self._trace("X9a.hard_stop", "skip", f"roi={roi:.1%} ≥ {stop_loss:.0%} 未启动止损层")
            self._trace("X9b.cond_stop", "skip", "父闸未开启")
            return None

        abs_sl = float(self._xp(pos, "ABSOLUTE_STOP_LOSS"))
