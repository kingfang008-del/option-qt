...
from anchor_contract_utils import (
    merge_exec_bucket_quotes,
    option_exec_horizon_bars,
    resolve_anchor_config,
    load_bucket_minute_quotes,
)
...
# 配置文件路径（优先 New_Pro slow_feature.json）
_SCRIPT_DIR = Path(__file__).resolve().parent
CONFIG_FILE = _SCRIPT_DIR.parent / "CONFIG" / "slow_feature.json"
if not CONFIG_FILE.exists():
    CONFIG_FILE = Path.home() / "notebook/train/feature_all.json"


def feature_names_for_resolution(config: dict, resolution: str) -> list:
    """按 resolution 字段筛选应写入该分辨率 parquet 的特征列。"""
    names = []
    for f in config.get('features', []):
        res = f.get('resolution', '1min')
        if res in (resolution, 'both'):
            names.append(f['name'])
    return names


def expected_option_columns(config: dict, resolution: str) -> list:
    """该分辨率下、应从 bucketed 期权 parquet merge 进来的列。"""
    return [
        f['name'] for f in config.get('features', [])
        if f.get('calc') in ('options', 'options_chain')
        and f.get('resolution', '1min') in (resolution, 'both')
    ]
...
class FeatureEngineer:
    ...
    def compute_features_raw(self, df: pd.DataFrame, resolution: str):
        ...
        final_feature_cols = feature_names_for_resolution(self.config, resolution)
        ...
        # --- 4. 最终列选择（按 resolution 过滤，1min/5min parquet 各写各的列）---
        final_feature_cols = feature_names_for_resolution(self.config, resolution)
        missing_cols = [col for col in final_feature_cols if col not in df.columns]
        if missing_cols:
            logging.warning(
                f"分辨率 {resolution} 缺少 {len(missing_cols)} 个配置特征: "
          ...
          # 中间循环遍历 time_range 和对应的 resolutions 列表
            ...
            for time_range, resolutions in time_ranges.items():
                ...
                for res in resolutions:
                    ...
                    if option_df is not None and not option_df.empty:
                        missing_opts = [
                            c for c in expected_option_columns(config, res)
                            if c not in merged_df.columns
                        ]
                        if missing_opts:
                            logging.warning(
                                f"[{symbol}/{year_month}/{res}] bucketed 期权缺少列 {missing_opts}。"
                                f"请确认 options_locked_feature.py 已重跑: {std_path}"
                            )
...
def update_new_labels_in_files(config: dict):
    ...
    for res in results:
        ...
        if status == "success":
            ...
            net_ratio = res.get("net_consol_ratio", ratio)
            avg_cost = res.get("avg_execution_cost", 0.0)
            vol_ratio = res.get("volume_ratio", 0.0)
            
            success_stats.append((ratio, vol_ratio, net_ratio, avg_cost))
    ...
    def avg_net_consol_ratio(bucket):
        if not bucket: return 0.0
        return np.mean([s[2] for s in bucket])

    def avg_execution_cost(bucket):
        if not bucket: return 0.0
        return np.mean([s[3] for s in bucket]) * 10000
    ...
    if total_success > 0:
        ...
        print(f"  - Net盘整均值 : {avg_net_consol_ratio(success_stats):5.1f}% | AvgCost: {avg_execution_cost(success_stats):6.2f} bp")
        print("="*80 + "\n")
...
def _labeling_param(config: dict, name: str, default):
    """Read a labeling parameter from config/env with a stable default."""
    env_name = f"LABEL_{name.upper()}"
    if env_name in os.environ:
        raw = os.environ.get(env_name)
    else:
        raw = (config or {}).get('parameters', {}).get('labeling', {}).get(name, default)
    try:
...
def _estimate_round_trip_cost_series(df: pd.DataFrame, config: dict, valid_len: int) -> pd.Series:
    """
    Estimate execution friction for option-like labels.

    For BACKTEST_OPT_FILL_SPREAD_FRAC=0.75, the expected round-trip drag is
    roughly 0.5 * spread_pct when both sides are filled inside the spread.
    """
    ...
    bid_col = _first_existing_col(df, [
        "exec_call_bid", "options_bid", "option_bid", "call_bid", "atm_call_bid", "bid",
        "options_call_bid", "options_atm_call_bid",
    ])
    ask_col = _first_existing_col(df, [
        "exec_call_ask", "options_ask", "option_ask", "call_ask", "atm_call_ask", "ask",
        "options_call_ask", "options_atm_call_ask",
    ])
    px_col = _first_existing_col(df, [
        "exec_call_mid", "options_mid", "option_mid", "mid_price", "call_price", "atm_call_price",
        "options_call_price", "options_atm_call_price",
    ])

    cost = pd.Series(floor, index=df.index, dtype="float64")
    if bid_col and ask_col:
        bid = pd.to_numeric(df[bid_col], errors="coerce")
        ask = pd.to_numeric(df[ask_col], errors="coerce")
        if px_col:
            mid = pd.to_numeric(df[px_col], errors="coerce").replace(0, np.nan)
        else:
            mid = ((bid + ask) / 2.0).replace(0, np.nan)
        spread_pct = ((ask - bid) / mid).clip(lower=0.0, upper=0.20).fillna(0.0)
        inside_drag = max(0.0, 2.0 * (fill_frac - 0.5))
        cost = pd.Series(np.maximum(floor, inside_drag * spread_pct), index=df.index)
    return cost.fillna(floor)


def _apply_executable_net_labels(
    df: pd.DataFrame,
    config: dict,
    gross_return: pd.Series,
    valid_len: int,
) -> tuple[float, float]:
    """Add cost-aware labels while preserving gross labels."""
    df["label_return_fwd_gross"] = gross_return.fillna(0.0)
    df["label_return_fwd_net"] = 0.0
    df["label_direction_net"] = 1
    df["label_execution_cost"] = 0.0
    if valid_len <= 0:
        return 100.0, 0.0

    cost = _estimate_round_trip_cost_series(df, config, valid_len)
    idx = df.index[:valid_len]
    gross = gross_return.loc[idx].fillna(0.0).astype("float64")
    cost_valid = cost.loc[idx].astype("float64")
    net = np.sign(gross) * np.maximum(np.abs(gross) - cost_valid, 0.0)
    flat_margin = float(_labeling_param(config, "executable_flat_margin", 0.0005))
    net_dir = np.ones(len(idx), dtype=np.int8)
    net_dir[net > flat_margin] = 2
    net_dir[net < -flat_margin] = 0

    df.loc[idx, "label_return_fwd_net"] = net
    df.loc[idx, "label_direction_net"] = net_dir
    df.loc[idx, "label_execution_cost"] = np.asarray(cost_valid, dtype=np.float64)
    consol_ratio = float((net_dir == 1).mean() * 100.0)
    avg_cost = float(cost_valid.mean()) if len(cost_valid) else 0.0
    return consol_ratio, avg_cost


def process_labels_file(file_path: Path, config: dict = None) -> dict:
    """
    [New Delta 架构 - 30min Horizon 适配版]
    QQQ 0DTE profile: return_fwd / return_fwd_net 改用锁定 CALL ATM 期权 PnL（与 option_exec_label 对齐）。
    """
    try:
        # 1. [I/O] 读取数据
        ...
        anchor_cfg = resolve_anchor_config(config)
        use_option_exec_labels = False
        symbol = None
        if anchor_cfg:
            try:
                symbol = file_path.parts[file_path.parts.index(OUTPUT_FEATURES_DIR.name) + 1]
            except (ValueError, IndexError):
                symbol = None
            if symbol:
                df = merge_exec_bucket_quotes(df, symbol, anchor_cfg)
                use_option_exec_labels = df.get("exec_call_mid", pd.Series(dtype=float)).notna().any()
        ...
        if use_option_exec_labels:
            entry_bars, hold_bars = option_exec_horizon_bars(config or {})
            k_slow = hold_bars
            horizon_bars = entry_bars + hold_bars
        else:
            k_slow = 30
            horizon_bars = k_slow
            entry_bars = 0
        ...
        n = len(df)
        valid_len = max(0, n - horizon_bars)
        ...
        if valid_len > 0:
            ...
            if use_option_exec_labels and 'exec_call_mid' in df.columns:
                mid = pd.to_numeric(df['exec_call_mid'], errors='coerce')
                entry_px = mid.shift(-entry_bars)
                exit_px = mid.shift(-(entry_bars + hold_bars))
                gross_return = (exit_px / entry_px.replace(0, np.nan) - 1.0).fillna(0.0)
                opt_cfg = (config or {}).get('parameters', {}).get('option_exec_label') or {}
                flat_margin = float(opt_cfg.get('min_edge', 0.02))
                opt_ret = gross_return.iloc[:valid_len].to_numpy()
                dir_labels = np.ones(valid_len, dtype=int)
                dir_labels[opt_ret > flat_margin] = 2
                dir_labels[opt_ret < -flat_margin] = 0
                df.iloc[:valid_len, df.columns.get_loc('label_direction')] = dir_labels
            else:
                gross_return = (df[price_col].shift(-k_slow) / df[price_col] - 1.0).fillna(0.0)

            df['label_return_fwd'] = gross_return
            net_consol_ratio, avg_execution_cost = _apply_executable_net_labels(df, config or {}, gross_return, valid_len)
            ...
            df['label_return_fwd_gross'] = 0.0
            df['label_return_fwd_net'] = 0.0
            df['label_direction_net'] = 1
            df['label_execution_cost'] = 0.0
            net_consol_ratio = 100.0
        ...
        return {
            ...
            'net_consol_ratio': net_consol_ratio,
...
def process_option_labels_file_vectorized(feature_file_path: Path, config: dict):
    ...
    try:
        ...
        anchor_cfg = resolve_anchor_config(config)
        use_bucket_chain = anchor_cfg is not None

        if use_bucket_chain:
            bucket_id = int(anchor_cfg.get("label_trade_bucket_id", 2))
            quotes = load_bucket_minute_quotes(
                stock_symbol, feature_df["timestamp"], bucket_id, anchor_cfg
            )
            if quotes.empty:
                return {"status": "skipped", "message": f"无 bucket {bucket_id} 日级 sniper/IV 数据"}
            option_df = quotes.copy()
            option_df["contract_symbol"] = f"BUCKET_{bucket_id}"
            option_df["mid_price"] = option_df["exec_call_mid"]
            option_df["option_type"] = "call"
            option_df["dte"] = 0.0
            if "iv" not in option_df.columns:
                option_df["iv"] = np.nan
            if "gamma" not in option_df.columns:
                option_df["gamma"] = np.nan
            if "theta" not in option_df.columns:
                option_df["theta"] = np.nan
            if "delta" not in option_df.columns:
                option_df["delta"] = np.nan
        else:
        ...
        # 处理期权链数据 (DTE 计算) — 0DTE profile 已锁定 bucket，跳过链式筛选
        if not use_bucket_chain:
            ...
            calls_df = option_df.copy()
            if calls_df.empty:
                return {"status": "skipped", "message": "无 0DTE bucket 报价"}
