...
SOURCE_DIR = Path.home() / "train_data/features_raw" 

# 【必须修改】定义新的、分离的训练集和验证集输出目录
TRAIN_DIR = Path.home() / "train_data/features_train"
VAL_DIR = Path.home() / "train_data/features_val"

# 定义训练集和验证集的时间范围 (左闭右闭)
TRAIN_DATE_RANGE = ("2022-03-01", "2025-04-30") 
VAL_DATE_RANGE = ("2025-05-01", "2025-12-31")
...
def get_destination_path(file_path: Path, source_base: Path, train_base: Path, val_base: Path, 
                         train_range: Tuple[pd.Timestamp, pd.Timestamp], 
                         val_range: Tuple[pd.Timestamp, pd.Timestamp]) -> Path | None:
    """
...
def process_and_copy_file(file_path: Path, source_dir: Path, train_dir: Path, val_dir: Path, 
                          train_range_ts: Tuple, val_range_ts: Tuple) -> str:
    """
    单个文件处理的工作函数
    """
    dest_path = get_destination_path(file_path, source_dir, train_dir, val_dir, train_range_ts, val_range_ts)
